...
jQuery('#SaveSolrSearch').bind('click',saveSolrSearch);
    function saveSolrSearch (e){
        e.preventDefault();
        var oThis = jQuery(this);
        var myncbiurl = oThis.attr('href');
	   	var args = {
            'QK': oThis.data('qk'),
            'HID': oThis.data('hid'),
            'p$rq': oThis.data('pname') + ':CreateQueryForSaveSolrSearch'
        };
        ...
        jQuery.ajax({
            ...
            if(status == 408){
                //display an error indicating a server timeout
                 alert('Create alert timed out.');
            }
            
            var JSONobject = eval('(' + response + ')');
            // display link to feed
            newURL = myncbiurl + JSONobject.NewQueryKey;
            window.location = newURL;
            
            
        }).fail(function(){
        	 alert('Create alert unavailable.');
        });
    }  
...
Portal.Portlet.Pubmed_DisplayBar = Portal.Portlet.Entrez_DisplayBar.extend({
	...
	'SendToClick': function(sendto, idx,  e, target, name) {
	    var count = this.getItemCount();
	    if (jQuery('#issolrsearchflag').val() == 'true' && count > 10000 && sendto.toLowerCase() == 'file' ){
	        jQuery('#submenu_' + sendto).hide();
	        alert('Please select a Sort order other than "Best Match" to save beyond 10,000 citations.');
	        this.ResetSendToSelection();
	    }
        else if (sendto.toLowerCase() == 'order'){
	    }
			...
			if (!CaptchaReady){
		    	...
		    	jQuery.cachedScript( CaptchaScript) // add timeout here?
					...
					.fail(function( jqxhr, settings, exception ) {
						jQuery("#email_alert").html("Please <a href='/account/?back_url=" + encodeURIComponent(CurrURL) +
						                            "'>sign in</a> to your NCBI account to send email").attr('class', 'email_alert');
					});
			}
			else{
				this.GetCaptcha();
	...
	'SendToEmailSubmitted': function(cmd, target){
	    ...
	    if (jQuery('#issolrsearchflag').val() == 'true' && document.getElementById("email_start") && document.getElementById("email_start").value > SolrSearchResultLimit){
	        alertnode.innerHTML = 'Please select a Sort order other than "Best Match" to enter a "Start from citation" number beyond 10,000.';
	        alertnode.className = 'email_alert';
	    }
	    else if (email == ''){
    		...
    		if (emailRegexp.test(email)){   		    
	        	...
	        	if (document.getElementById("captcha-segment")){
	        		...
	        		if (captchaId == undefined){
	        			jQuery("#email_alert").html("We could not verify that you are not a robot. Please <a href='/account/?back_url=" + 
	        			...
	        			if (captchaanswer != ''){
        	    	    	...
        	    	    	alertnode.innerHTML = 'Please verify that you are not a robot or <a href="/account/?back_url=' + 
        	    	    	                        encodeURIComponent(CurrURL) +
						                            '">sign in</a> to your NCBI account to send email';
	                        alertnode.className = 'email_alert';
	        			}
        			}
	        	} 
	...
	'SendMailInfo': function(cmd, target, email, captchaanswer){
	    ...
	    var captchaexpt = target.getAttribute('expt');
    		...
    		if (jQuery("#recaptcha_div").find("div") && CaptchaId != undefined){
		        ...
		        NCaptchaId = grecaptcha.render(
			  	...
			  	if (NCaptchaId != undefined){
			  		...
			  		jQuery("#email_alert").html("We are experiencing some problems verifying that you are not a robot. Please try again later or <a href='/account/?back_url=" + 
	        				                            encodeURIComponent(CurrURL) +
						                                "'>sign in</a> to your NCBI account to send email.").attr('class', 'email_alert');		
    ...
    'MailSubmissionResponse':function(data){
    	...
    	if (data.success == 'true'){
    	    ...
    	    this.send.RemoveUserMessage({'name' : 'email_m'});
    	    var CurrURL = window.location.href;
    		jQuery("#email_alert").html("We could not verify that you are not a robot. Please try again or <a href='/account/?back_url=" + 
	        				                            encodeURIComponent(CurrURL) +
						                                "'>sign in</a> to NCBI.").attr('class', 'email_alert'); 
	    	document.getElementById("email_submit").disabled = false;
	    	//reissue new captcha
    		this.GetCaptcha();
    	}
    ...
    'SendToCitManagerSubmitted': function(cmd, target){
        if (jQuery('#issolrsearchflag').val() == 'true' && document.getElementById("citman_start_option") && document.getElementById("citman_start_option").style.display == "list-item" && document.getElementById("citman_start").value > SolrSearchResultLimit){
            alert('Please select a Sort order other than "Best Match" to enter a "Start from citation" number beyond 10,000.');
        }
        else{
    ...
    "getPortletPath" : function(){
	    return (this.realname + ".Entrez_DisplayBar");
	}
...
var CaptchaRef;
var SolrSearchResultLimit = 10000;
...
var AbstractLink = function (){
	...
	var eUtils = "https://eutils.ncbi.nlm.nih.gov/eutils";
	...
	var secDb = {
		...
		"ssid": ["PubMed", "ANZCTR", "BioProject", "ChiCTR", "ClinicalTrials.gov", "CRiS", "CTRI", 
		        "Dataset DOI", "dbGaP", "dbSNP", "dbVar", "DRKS", "Dryad", "EudraCT", "figshare", "GEO",
                "IRCT", "ISRCTN Controlled Trials", "JapicCTI", "JMACCT", "JPRN", "NTR", "Nucleotide", "OMIM",
                "PACTR", "Peruvian Clinical Trial Registry", "Protein",
                "PubChem-BioAssay", "PubChem-Compound", "PubChem-Substance", "ReBec", "RPCEC",
                "SLCTR", "SRA", "Structure", "SWISSPROT", "TCTR", 
                "UMIN-CTR", "UniMES", "UniParc", "UniProtKB", "UniRef"],
		"grnt": ["PubMed"],
...
/*  		else if (currDb.match(/PubChem\s+(\S+)/) && term.match(new RegExp('^PubChem-(' + RegExp.$1 + ')\/(\\d+)'))) {
		...
		else if (currDb == "pccompound" && term.match(/^PubChem-Compound\/(\d+)/)){ //|| currDb == "pcassay" || currDb == "pccompound")
		     return "https://pubchem.ncbi.nlm.nih.gov/compound/" + RegExp.$1;
		}
  		...
  		else if(currDb == "Dataset DOI" && term.match(/^Dataset DOI\/(.+)/i)){
  		    return "https://doi.org/" + RegExp.$1;
  		}
  		    ...
  		    return "http://doi.org/" + RegExp.$1.toLowerCase();
  		}
  		    ...
  		    return "http://doi.org/" + RegExp.$1;
  		}
   		else if(currDb == "IRCT" && term.match(/^IRCT\/(.+)/i)){
  		    return "http://www.irct.ir/searchen.php?field=a&lang=en&keyword=" + RegExp.$1;
  		}
  		else if(currDb == "JapicCTI" && term.match(/^JapicCTI\/(.+)/i)){
  		    return "http://www.clinicaltrials.jp/user/showCteDetailE.jsp?japicId=" + RegExp.$1;
  		}
   		else if(currDb == "JMACCT" && term.match(/^JMACCT\/(.+)/i)){
			...
			if (currDb == "Add to Search") {
				...
				if (link) {
					...
					if (jQuery.inArray(currDb.toLowerCase(),ncbiResouces) != -1)
					    retArr.push('{"text":"Search in ' + currDb.replace(/-/, ' ') + '","href":"' + link + '"}');
					else
...
(function($){
    ...
    $(function(){
        ...
        var fullUrl = (discCol.find("a").attr("href").replace("&page=full","")).split('?');
