YUI.add('portal2012dispatcher', function(Y) {
...
//  Frequently used strings
        ...
        SC = "script",
        SC_NOT_JSON = ':not(script[type="application/json"])',
...
//  Utility functions
    function _parseContent(content, normalize) {
        ...
        if (normalize && (match = reHEAD.exec(content))) {
            fragment.setContent(match[1]).all(SC+',style,link[type=\'text\\css\']').each(function(n) {
                head.append(n);
            });
        ...
        o.js = fragment.all(SC).filter(SC_NOT_JSON).each(function(n) {
        ...
        ATTRS: {
            ...
            node: {
                ...
                setter: function(n) {
                    ...
                    this.stop();
                    return typeof n == typeof Y.Node ? n : Y.one(n);
                }
            },
            /**
...
}, 'portal2012dispatcher-01' ,{requires:['base', 'node-base', 'io-base', 'get', 'async-queue', 'classnamemanager']});
