...
(function ($) {
	...
	com.nature.AlertMessage = {
		...
		hideMessage: function (ev, message) {
			...
			$msgContainer.find(".blurb").slideUp('slow', function () {
				$msgContainer.find("." + scope).removeClass("visible").addClass("minimised");
			});

			$msgContainer.find(".message-control a.persistent").html("Find out more<span class='hidden'> about this message</span>");
		},
		...
		showMessage: function (ev, message) {
			...
			$msgContainer.find(".blurb").slideDown('slow', function () {
				$msgContainer.find("." + scope).removeClass("minimised").addClass("visible");
			});
			$msgContainer.find(".message-control a.persistent").text("Minimise this message");
		}
