...
var DynamicArtContent = {
  ...
  buildDynamicRequestData: function() {
    ...
    requestObj.excerptDisplay = SDM.excerptDisplay;
    ...
    DynamicArtContent.fragReady = false;
    DynamicArtContent.ajaxCallInFlight = false;
    DynamicArtContent.retrieve();
  },
  retrieve: function() {
    DynamicArtContent.ajaxCallInFlight = true;
    var jqxhr = $.post(SDM.urlPrefix+"/dc/article/all", { data:DynamicArtContent.requestData }, function(res) {
        try {
            SDM.pf.dc.respObj = $.parseJSON(res);
            DynamicArtContent.ajaxCallInFlight = false;
            if (!Fragment.isAvail() || DynamicArtContent.fragReady) {
                DynamicArtContent.fragReady = true;
                DynamicArtContent.processResponse();
            }
        } catch (e) {
            DynamicArtContent.ajaxCallInFlight = false;
            DynamicArtContent.retryRetrieve();
        }
    });

    jqxhr.fail(function() {
        DBG.out (0, SDM.urlPrefix+"/dc/article/all POST call failed.");
        DynamicArtContent.ajaxCallInFlight = false;
        DynamicArtContent.retryRetrieve();
    });
  },
  retryRetrieve: function() {
      if( !DynamicArtContent.ajaxCallInFlight && 
         (DynamicArtContent.retryNum < SDM.pf.dc.retry)) {
  ...
  processResponse: function() {
      try {
        if (SDM.pf.dc.respObj.DynamicComponentsResp == undefined ||
	   SDM.pf.dc.respObj.DynamicComponentsResp == null) {
            DynamicArtContent.retryRetrieve();
        } 
        else {
          DBG.out(0, SDM.pf.dc.respObj);
          DynamicArtContent.dynamicComponents = SDM.pf.dc.respObj.DynamicComponentsResp;
  ...
  populateRefModFields:function() {
      if(DynamicArtContent.dynamicComponents.moduleAvail != undefined && DynamicArtContent.dynamicComponents.moduleAvail =="Y"){
         ...
         $('#mrwModuleSrcTitle').removeClass('hidden');
  ...
  buildPdfExcerptHtml: function(scope) {
    ...
    $('.excerptLink,.excerptLink1').unbind('click').bind('click', function(e) { var nw = window.open($('.excerptLink').attr('href'), '_blank', 'height=' + $(window).height()*.9 +',width=' + $(window).width()*.9+',resizeable=yes,left=50,top=50'); e.preventDefault(); e.stopPropagation();});
  ...
  buildAccessLinks: function(scope) {
    ...
    /* if((DynamicArtContent.dynamicComponents.CorpReqAvail != undefined  && !SDM.ignoreForceAbst)){ */
        ...
        if ( DynamicArtContent.dynamicComponents.AnonLinkAvail != undefined ) {
            ...
            if ( DynamicArtContent.dynamicComponents.RemoteLinkURL != undefined ) {
                accessHTML += '<a title="Apply for remote access" href="';
                accessHTML += DynamicArtContent.dynamicComponents.RemoteLinkURL;
                accessHTML += '"><br>Apply for remote access</a>';
            }
            accessHTML += '</div>'; 
            col1=true; 
        }
        if ( DynamicArtContent.dynamicComponents.CorpReqAvail != undefined && SDM.pageType != 'article_full' && !SDM.ignoreForceAbst) {
            accessHTML += '<div class="acol1">';  
            accessHTML += '<a title="Click to see if you have full text access" class="corpReq cLink lightGreenBtn S_C_fullText" queryStr="?_fmt=full" href="'
            accessHTML += SDM.urlPrefix + '/article/pii/' + SDM.pm.pii;
        ...
        if ((DynamicArtContent.dynamicComponents.CorpReqAvail == undefined) && 
            (DynamicArtContent.dynamicComponents.AnonLinkAvail == undefined)) {
          if ( DynamicArtContent.dynamicComponents.RemoteLinkURL != undefined ) {
            accessHTML += '<div class="acol1">';
            accessHTML += '<a title="Apply for remote access" href="';
            accessHTML += DynamicArtContent.dynamicComponents.RemoteLinkURL;
        ...
        accessHTML += '</div></div>';
        if(col1 || col2 || col3){
            $(scope + ' #ppvPlaceHolder').html (accessHTML).removeClass('hidden');
    ...
    if(outLinkHTML[4]!=''){
        $('h2.citedByLink').html(outLinkHTML[4]).show();
        $('h2.citedByLink').closest('.entry').show();
    }
    else{$('h2.citedByLink').closest('.entry').hide();}
    
    if(outLinkHTML[5]!=''){
        if($('.rightsLink').length) {
          $('.rightsLink').html(outLinkHTML[5]);
          $('.extLinks').addClass('nonEmpty');
        }
        else
          SDM.dac.rightsLink = outLinkHTML[5];
    }
    else{
        $('.innerPaddingFixedBox .permissions').hide();
        $('.moreInformation .permissions').hide();
    }
  }, 
  buildOutwardLinks:function(obj){
    var accessHTML = '';
        ...
        if(obj.LinkURL!=undefined && obj.LinkURL!=''){
            accessHTML += '<a class="S_C_outLinks" href="'+obj.LinkURL+'" target="_blank"';
  ...
  populateRefersToAndReferredToBy: function() {
    ...
    $.post(SDM.refLinkURL, { data:requestData }, function(res) {
      ...
      if(typeof ArticlePage != 'undefined')ArticlePage.viewHandler();
    });
  } 
}; 
...
var socialLogin = {
  init: function(containerID) {
	...
	$(targetElm).each(function(){
        if($(this).attr('title')=="OpenID"){
            $(this).attr('title','Use your '+$(this).attr('title')+' username to sign in to ScienceDirect');
        }else{
            $(this).attr('title','Use your '+$(this).attr('title')+' account to sign in to ScienceDirect');
        }
...
//    this.scrollFlag=true;
  ...
  addFrag: function(fI, fC, cfg) {
    ...
    if(!$('#' + fID).length) {
      ...
      var singleFrag = false;
      Fragment.processFrag($('#' + fID), singleFrag);
    }
    ++fI;
    if(fI<fC) {
  ...
  foldCheckJumper: function(fragStr) {
    var aCount = Fragment.mapAuthors.length;
    var rslt = 1;
    if(typeof console != 'undefined' && typeof console.time != 'undefined')console.time("authorLoop");
    for(var a=0; a<aCount; a++) {
      var fragIdStr = Fragment.getIntraRefFragId(Fragment.mapAuthors[a]);
      var fragId = fragIdStr.split('_')[1] * 1;
      if(fragId>rslt)
        rslt = fragId;
    }
    if(typeof console != 'undefined' && typeof console.timeEnd != 'undefined')console.timeEnd("authorLoop");
    Fragment.foldCheckJumper = function() {return rslt;};
    return rslt;
  },
  foldCheck: function(fragStr) {
    DBG.out(0, 'foldCheck(' + fragStr + ')');
    if(typeof fragStr == 'undefined') fragStr='frag_1';
    if( ($(window).height()*SDM.pm.frag.foldFactor) > ($('#centerInner')[0].scrollHeight*1.0) && ($('.page_fragment').last().attr('data-last')!='true') ) {
      var idInt = fragStr.substr(fragStr.indexOf('_')+1)*1;
      if(Fragment.mapsComplete) {
        var idInt2 = Fragment.foldCheckJumper() - 1;
        if(idInt2==0) {} //If no point in skipping let foldCheck behave as normal
        else if(idInt2 <= idInt) {} //If we are already past the skip behave as normal
        else {
          idInt = idInt2;
        }
        DBG.out(0, 'foldCheck::mapsComplete skipping to ' + idInt);
      }
      DBG.out(0, 'calling get for ' + (idInt+1));
      Fragment.get('frag_' + (idInt+1), undefined, {successCb: Fragment.foldCheck,
                                                    successCbArg: 'frag_' + (idInt+1),
                                                    showLoadedFrag: MoreInformation.hideFrag
                                                    });
    }
    else {
      Fragment.processHash();
      MoreInformation.init();
//      Fragment.scrollFlag=true;
  ...
  isMalformation: function() {
    return $('#centerInner').children().not('noscript, .page_fragment, .page_fragment_ind, #sdSystemMsg').length>0;
  },
  get: function(fragID, z, opts) {
    var cfg = {
      ...
      showLoadedFrag: function($f) {return true;}
    }
    if(opts) {
    ...
    var fragIDVal = fragID.substr(fragID.indexOf('_')+1);
    if (fragIDVal == 'NaN') {
      return;
    }
    ...
    if(fragID=='all') {
      ...
      if(Fragment.fragmentMap && typeof _.indexOf(Fragment.fragmentMap, fragID)=='undefined') {
        ...
        return;
              ...
              ($('.page_fragment').last().attr('data-fid')*1) <= (fragID.split('_')[1]*1)) {
  ...
  loadTarget: function(idStr, opt) {
    var allSet = false;
    if($('#'+idStr).length) {
      allSet = true;
      var fIdStr = Fragment.getFragId(idStr);
      var fId = fIdStr.split('_')[1];
      var allSet = true;
      for(var s=fId-opt.preBuffer; s<opt.preBuffer+opt.postBuffer+1; s++) {
        if(!$('#frag_'+s).length) {
          allSet = false;
        }
      }
    ...
    if(allSet) {
      ...
      var fIdStr = Fragment.getIntraRefFragId(idStr);
      if(!fIdStr) return;
      var fId = fIdStr.split('_')[1];
      var start = fId-opt.preBuffer;
      if(start<2) start = 2;
      var len = opt.preBuffer+opt.postBuffer+1;
      if(start+len>Fragment.fragmentMap.length) len = (Fragment.fragmentMap.length-start)+1;
      Fragment.getBatch({s: start,
                         l: len,
                         f: opt.f});
    }    
  },
  getBatch: function(opt) {
    var start   = opt.s;
    var len     = opt.l;
    var func    = opt.f;
    $.ajax({type:'GET',
            url:SDM.urlPrefix + "/frag/" + SDM.pm.pii + "/" + SDM.pm.fat + "/" + "batch" + "/" + start + "/" + len,
            success: function(str,b,c) {
              str.length > 30 ? sI=-30 : sI=str.length*-1;
              var endStr = str.slice(sI);
              if(endStr.match(/ERROR::/)) {
                DBG.out(0, "Fragment::fragLoad:: Unable to retrieve batch");
                return;
              }
              var parts = str.split('!!!FRAG!!!');
              var partCount = parts.length-1;
              DBG.out(0, "Fragment::fragLoad:: received " + partCount + " fragments");
              for(var i=0; i<partCount; i++) {
                DBG.out(1, "Fragment::fragLoad::working on " + (start+i));
                var fragIdNum = start+i;
                if(!$('#frag_'+fragIdNum).length) {
                  var prevFrag = Fragment.getPrevFrag(fragIdNum);
                  DBG.out(1, "Inserting after " + prevFrag.attr('id'));
                  prevFrag.after(parts[i]);
                  $('#frag_' + fragIdNum).show();
                  Fragment.processFrag($('#frag_' + fragIdNum), true);
                }
                else {
                  DBG.out(0, "ignoring frag_" + fragIdNum);
                }
              }
  ...
  getSuccess: function(fragStr, fragID, cfg, z) {
    ...
    var singleFrag = true;
    Fragment.processFrag($('#frag_' + fragIdNum), singleFrag);
  ...
  processFrag: function($frag, singleFrag) {
    ...
    ArticlePage.lazyLoadFragInit(fragID, singleFrag);
  ...
  processHash: function() {
    ...
    if (hash != 'undefined' && hash !='') {
      if(Fragment.isAvail() && !Fragment.fragmentMap) {
      ...
      if(hash == "MMCvFirst") {
        if($('dl.ecomponent').length ) {
          hash = $('dl.ecomponent').first().attr('id');
          if(typeof hash == 'undefined') {
            $('dl.ecomponent').first().attr('id', 'ecmptJumpId');
            hash = 'ecmptJumpId';
          }
        }
        else {
          hash = Fragment.mapEcomponents[0];
        }
      ...
      if(Fragment.isAvail()) {
        var $fragSet = $('#centerInner .page_fragment').filter(function(i) {return MoreInformation.hideFrag($(this));})
        $fragSet.show();
      }
      if($('#'+hash)[0]) {
        ...
        $('html,body').moveTo(hash, 
                              {cbFunc:function() {
                                   Fragment.scrollFlag=true;
                               }
                              });
      }
      else if (Fragment.isAvail()){
        var targetFrag = Fragment.getIntraRefFragId (hash);
        if(targetFrag) {
          Fragment.get ('frag_' + ((targetFrag.split('_')[1]*1)-1), undefined);
  ...
  scroll: function() {
    if(Fragment.scrollFlag) {
      var isDown = $(window).scrollTop() > Fragment.prevST;

      Fragment.prevST = $(window).scrollTop();
      
      var h = $(window).height();
      h = Math.floor(h * Fragment.winHeightAdjustFactor);
      var st = $(window).scrollTop();
      var sh = $(window)[0].scrollHeight;
      var needList = Fragment.processScroll(st, st+h, isDown, {filterFunc: function(i) {
                                                                if(MoreInformation.closed==true) {
                                                                  if( !MoreInformation.hideFrag($(this)) ) {
                                                                    return false;
                                                                  }
                                                                  return true;
                                                                }
  ...
  setMaps: function(str) {
    if(Fragment.mapsComplete) return;
  ...
  setCrossRefMap: function() {
    ...
    if(SDM.isRefLoad) { 
        ...
        if(fid) {
            fid = (fid.split('_')[1])*1;
            if(refFragId==-1 || fid<refFragId) {
                refFragId = fid;
            }
        }
    if(refFragId!=-1)
      Fragment.get('frag_' + refFragId);
    }

  },
  processScroll: function (visTop, visBot, isDown, cfg) {
    DBG.out(0, 'processScroll(' + visTop + ',' + visBot + ')');
    var fList = [];
    var $fragSet = $('#centerInner .page_fragment').filter(cfg.filterFunc)
    $fragSet.each(function() {
      DBG.out(5, "processScroll::loop::about to show " + $(this).attr('id'));
      $(this).show();
      ...
      if( (fragBot >= visTop && fragBot <= visBot) ||
        ...
        DBG.out(2, $(this).attr('id') + '::' + fragTop + '::' + fragBot);
      }
    });
    ...
    if(isDown) {
      DBG.out(3, 'before::' + fList);
      fList = Fragment.gapFindDown(fList);
      DBG.out(3, 'after::' + fList);
      if($('#frag_' + fList[fList.length-1]).attr('data-last')) {}
      else {
        var nfs = 'frag_' + (fList[fList.length-1]+1);
        DBG.out(1, 'Going down... looking for ' + nfs);
        if(!$('#'+nfs).length) {
          Fragment.get(nfs, 'centerPane', {showLoadedFrag: MoreInformation.hideFrag});
        }
        else {
          if(MoreInformation.hideFrag($(nfs))) $(nfs).show();
        }
      }
    }
    else {
      DBG.out(3, 'before::' + fList);
      fList = Fragment.gapFindUp(fList);
      DBG.out(3, 'after::' + fList);
      if(fList[0]!=1) {
        var nfs = 'frag_' + (fList[0]-1);
        DBG.out(1, 'Going up... looking for ' + nfs);
        if(!$('#'+nfs).length) {
          Fragment.get(nfs, 'centerPane', {
          successCb: function() {
            if(MoreInformation.hideFrag($(nfs))) {
              var nfsH = $('#'+nfs).height();
              var nst = $(window).scrollTop() + nfsH + 10;
              DBG.out(5, "processScroll::1::original scrollTop " + $(window).scrollTop());
              DBG.out(5, "processScroll::1::new      scrollTop " + nst);
          ...
          var isVis = $('#'+nfs).is(':visible');
          if(MoreInformation.hideFrag($(nfs))) {
            $(nfs).show();
            if(!isVis && $('#'+nfs).is(':visible')) {
              var nfsH = $('#'+nfs).height();
              var nst = $(window).scrollTop() + nfsH + 10;
              DBG.out(5, "processScroll::2::original scrollTop " + $(window).scrollTop());
              DBG.out(5, "processScroll::2::new      scrollTop " + nst);
              $(window).scrollTop(nst);
            }
          }
        }
    ...
    DBG.out(2, "processScroll is done");
  },
  gapFindDown: function(a) {
    var r = [];
    var l = a.length;
    var shift = 0;
    for(var i=0; i<l; i++) {
      ...
      if(i<l-1) {
        ...
        if(a[i]+1 == a[i+1]) {
          ...
          DBG.out(4, 'gapFindDown::found a gap::' + a[i+1]);
          a[i+1] = -1;
          need = a[i]+1;
          shift++;
          return r;
      ...
      if(a[i]!=-1) {
        if(i!=0) {
          ...
          if(a[i]-1 == a[i-1]) {
            ...
            DBG.out(4, 'gapFindUp::found a gap::' + a[i-1]);
            a[i-1] = -1;
            need = a[i]-1;
            shift++;
            while(typeof r[0] == 'undefined') {
              r.shift();
            }
            return r;
          }
  ...
  isFragmentMapLoaded: function() {
       if(typeof Fragment.fragmentMap == 'undefined') { return false; }
       else {return true;}
  },
  handleAipClicks: function(ee) {
      var selID=$(ee.target).closest('a').attr('href').split('#');
      selID=selID[1]; 
      ee.preventDefault();
      if (!Fragment.isAvail()) {
         $('html,body').moveTo(selID, {focus: false});
         return;
      }
      Fragment.scrollFlag=false;
      if(Fragment.isAvail()) {
      ...
      if (Fragment.isFragmentMapLoaded()) {
           if (!$('#frag_'+ Fragment.fragmentMap.length)[0]) {
           Fragment.get ('frag_' + Fragment.fragmentMap.length, null, {successCb:function() {
                                                                                    $('html,body').moveTo(selID, {focus: false,
                                                                                                                  cbFunc:function() {
                                                                                                                       Fragment.scrollFlag=true;
                                                                                                                  }});
                                                                       },
                                                                       failureCb: function() {Fragment.scrollFlag=true;}});
           } else {
               $('html,body').moveTo(selID, {focus: false, cbFunc:function() { Fragment.scrollFlag=true; }}); 
           }
       } else {
            if(Fragment.waitDone == false) {
                Fragment.waitDone = true;
                setTimeout(function() {Fragment.handleAipClicks(ee)}, SDM.pm.frag.hashWait);
            } else {
                Fragment.scrollFlag=true;
            }
      } 
  ...
  ArticlePage = function() {
    ...
    var page = {
      ...
      addDynamics: function(fragId) {
        ...
        if( $(scope + ' .doiLink').length && SDM.doi != undefined && SDM.doi != '') {
          var doiHtml='';
          doiHtml += '<dd class="doi">';
          doiHtml +=    '<span>';
          doiHtml +=     '<a id="ddDoi" class="S_C_ddDoi" href="http://dx.doi.org/' + SDM.doi + '" target="doilink" onclick="var doiWin; doiWin=window.open(' + "'http://dx.doi.org/" + SDM.doi + "','doilink','scrollbars=yes,resizable=yes,directories=yes,toolbar=yes,menubar=yes,status=yes'); doiWin.focus()" + '">' + 'http://dx.doi.org/' + SDM.doi + '</a>';
          doiHtml +=     '<a class="icon_doiLink" title="Click to see how to cite or link using DOI (Opens new window)" href="' + SDM.urlPrefix + '/help/doi.htm" target="sdhelp" onmouseover="window.status=' + "'Help is Available';return true" + '" onclick="var helpWin; helpWin=window.open(' + "'" + SDM.urlPrefix + "/help/doi.htm','sdhelp','scrollbars=yes,resizable=yes,directories=no,toolbar=no,menubar=no,status=no,width=760,height=570'); helpWin.focus()" + '"></a>';
          doiHtml +=     '</span>';
          doiHtml += '</dd>';
          $(scope + ' .doiLink').html(doiHtml);
          $('.extLinks').addClass('nonEmpty');
        }
        if(SDM.dac.rightsLink) {
          $(scope + ' .rightsLink').html(SDM.dac.rightsLink);
          $('.extLinks').addClass('nonEmpty');
        }
        
        //all done if this was a fragment
        if(typeof fragId!='undefined' && fragId != 'frag_1' ) {return;}
        if(!dynamicsAdded){
            AuthorAccess.init();
        
            $('li.collabWithAuthorsCollapse a.collabText').bind('click',function(e){page.hideShowCollab(e,this,true);} );
            ...
            if(SDU.isIE78()) {
              ...
              $('#articleToolbar').addClass('toolbarFix');
            }
            else {
              if(SDU.isIE9())$('#articleToolbar').addClass('toolbarFix');
              $(window).resize(_.debounce(page.setPaneWidths, 500));
            }
            page.setPaneWidths();
            setQuickSearch();
            $(window).resize(function(){ArticlePage.setDefaultWidths();setQuickSearch();}); 
            $(window).scrollTop(0);
		    dynamicsAdded = true;
        }
      },
      ...
      toggleOptions:function(e,obj) {
        ...
        $(window).bind('scroll',ArticlePage.closeOptions);
        SD_UTIL.killEvent(e); 
      },
      ...
      hideShowCollab:function(e,obj,startClosed) {
        if(startClosed) {
         if($(obj).hasClass('collabOpened')) {
           $(obj).removeClass('collabOpened').addClass('collabClosed');
         }
         else {
           $(obj).removeClass('collabClosed').addClass('collabOpened');
         }
        }
     ...
     setDefaultWidths:function(){
        $('.appContainer.fixedPane').css('visibility','hidden');
        $('body').css('min-width','1240px');
        $('#rightPane').css('width','300px');
     },
      setPaneWidths: function () {
        var vpHeight = $(window).height();
        if($(window).height()>$('body').height()){
            pgLayout.vpWidth = $(window).width()-$.scrollBarWidth();
        }else{
            pgLayout.vpWidth = $(window).width();
        }
        /*reset to default values*/
        pgLayout.lcWidth = 260;
        pgLayout.ccWidth = 680;
        pgLayout.rcWidth = 300;
        pgLayout.lcLeft = 3;
        $('body').css({'max-width':'','min-width':''});
        if (1366 <= pgLayout.vpWidth) {
            pgLayout.vpWidth=1366;
            pgLayout.lcWidth = 260;
            $('#leftButton,#leftPane,#outlineContainer,.articleInternal').removeClass('small off').css('top','');
            pgLayout.rcWidth = pgLayout.vpWidth - pgLayout.lcWidth - pgLayout.ccWidth-1;
            pgLayout.showLeftBar=true;
        }else if (1280-$.scrollBarWidth() <= pgLayout.vpWidth) {
            pgLayout.lcWidth = 260;
            $('#leftButton,#leftPane,#outlineContainer,.articleInternal').removeClass('small off').css('top','');
            pgLayout.rcWidth = pgLayout.vpWidth - pgLayout.lcWidth - pgLayout.ccWidth-1;
            pgLayout.showLeftBar=true;
        }else if (1024-$.scrollBarWidth() <= pgLayout.vpWidth) {
            if(pgLayout.vpWidth<1240){
                pgLayout.lcWidth = 28;
                if(pgLayout.vpWidth<1008)pgLayout.vpWidth=1008;
                $('#leftButton,#leftPane,#outlineContainer,.articleInternal').removeClass('on').addClass('small');
                $('body').css('max-width','1074px');
        ...
        if(pgLayout.showLeftBar==false) {
          $('div#articleNav').css('margin-left',  6);
        }
        else{
          $('div#articleNav').css('margin-left',  237);
        }
        ...
        $('#rightPane,.appContainer.fixedPane').css('width',pgLayout.rcWidth);
        $('#leftPaneInner').not('.fixedPane').height(vpHeight-($('#header-area').height()-$('body').scrollTop())-$('#articleToolbar').outerHeight()-2);
        $('#leftPaneInner.fixedPane').width($('#leftPane').width());
        $('#rightOuter').height($('#rightInner').height());
        if($('#rightInner').height()+$('.appContainer.fixedPane').height()>$('#centerPane').height()){
            if($('#rightInner').height()>$('#page-area').height()){
                $('#rightOuter').height($('#rightInner').height()-1);
            }else{
                $('#rightOuter').height($('#page-area').height()-$('#articleToolbar').outerHeight()-1);
            }
        }else{
            if($('#centerPane').height()>vpHeight){
                $('#rightOuter').height($('#centerPane').height()-1);
            }else{
                $('#rightOuter').height(vpHeight-($('#header-area').height()-$('body').scrollTop())-$('#articleToolbar').outerHeight()-1);
            }
        }
        page.viewHandler();
      },
      getCurrentImg:function(){
        ...
        for(i=0;i<totalCount && loopBool==true ;i++){
            var obj = $(imgSel)[i];
            if($(obj).positionA('#centerContent').top+$(obj).height()>=$(window).scrollTop() &&
               $(obj).positionA('#centerContent').top<=$(window).scrollTop()+$(window).height()){
                var imgObj = $(obj);
                loopBool=false;
            }
        }
      ...
      lazyLoadInit: function() {
        $("img.imgLazyJSB").show();
        $(window).lazyLoadImages({imgSel:'img.imgLazyJSB'});  
        $('#outline').lazyLoadOutlineImages({imgSel:'img.smlImgLazyJSB'});  
      },
      lazyLoadFragInit: function(fragId, singleFrag) {
        $("img.imgLazyJSB").show();
        if (fragId != "all" && singleFrag) {
            $('#'+fragId).lazyLoadImages({imgSel:'img.imgLazyJSB'});
        }
      },
      viewHandler: function( e ){
        $('body').css({'min-width':''});
        $('.appContainer').css('visibility','');
        $(window).off( "scroll.viewHandler", page.viewHandler );
        var h = $(window).height();
        var st = $(window).scrollTop();
        DBG.out(1, 'viewHandler::window.height()::' + h + '  window.scrollTop()::' + st);
        var toolHeight=$('#articleToolbar').outerHeight();
        var toolTop=$('#articleToolbar').offset().top;
        var prevTop=$('#articleToolbar').attr('data-top');
        
        if(st>=toolTop && prevTop==undefined){
            $('#articleToolbar').attr('data-top',toolTop);
            $('#articleToolbar,.topBorder,#leftPaneInner').addClass('fixedPane');
            $('#articleToolbarDummy').show().css('height',toolHeight);
            $('#leftPaneInner').addClass('fixedPane').width($('#leftPane').width());
            $('#outlineContainer').css({'top':0});
        }else if(prevTop!=undefined && st<prevTop) {
            $('#articleToolbar').removeAttr('data-top').removeClass('fixedPane').css('left','');
            $('#leftPaneInner').removeClass('fixedPane').css({'width':''}).css('left','');
            $('.topBorder').removeClass('fixedPane').css('left','');
            $('#leftPaneInner').css({'top':''});
            //$('#leftPaneInner,#leftPane').height(h-($('#header-area').height()-$('body').scrollTop())-toolHeight-2);
            $('#leftPaneInner,#leftPane').height( h - ($('#header-area').height()+toolHeight) );
            $('#articleToolbarDummy').hide();
        }else{
            DBG.out(1, 'setting leftPane height::' + h + ' - (' + $('#header-area').height() + '-' + $('body').scrollTop() + ') - ' + toolHeight + '-' + 2);
            //$('#leftPaneInner,#leftPane').height(h-($('#header-area').height()-$('body').scrollTop())-toolHeight-2);
            $('#leftPaneInner,#leftPane').height( h - ($('#header-area').height()+toolHeight) );
        }
        $('#leftPaneInner.fixedPane').height(h-toolHeight);
        $('#leftPaneInner.fixedPane,.topBorder.fixedPane,.appContainer.fixedPane').css('top',toolHeight);
        $('.fixedPane').not('#articleToolbar').each(function(){
            var leftPos=$(this).parent().position().left
            $(this).css({
                'left': leftPos-$(document).scrollLeft()
            });
        });
        $('#rightOuter').height($('#rightInner').height());
        if($('#rightInner').height()+$('.appContainer.fixedPane').height()>$('#centerPane').height()){
            if($('#rightInner').height()>$('#page-area').height()){
                ...
                $('#rightOuter').height($('#page-area').height()-toolHeight-1);
            }
        }else{
            if($('#centerPane').height()>h){
                $('#rightOuter').height($('#centerPane').height()-1);
            }else{
                $('#rightOuter').height(h-($('#header-area').height()-$('body').scrollTop())-toolHeight-1);
            }
        }
        if($('li.optionsPos')[0])$('#erToolbar > div').css('padding-left',$('li.optionsPos').position().left-10);
        if($('.appContainer')[0]){
            var appTop=$('.appContainer').offset().top;
            var prevScrollTop=$('.appContainer').attr('data-scroll');
            if(typeof prevScrollTop=='undefined')prevScrollTop=0;
            var scroll_delta = 0
            if(prevScrollTop<st)scroll_delta=st - prevScrollTop;
            $('.appContainer').attr('data-scroll',st)
            if($('#articleToolbar.fixedPane')[0]){
                appTop-=$('#articleToolbar.fixedPane').outerHeight();
            }
            var prevAppTop=$('.appContainer').attr('data-top');
            $('.appContainerDummy:visible').css('height',$('.appContainer').outerHeight());
            if(st>=appTop-scroll_delta && prevAppTop==undefined){
                $('.appContainer').css('top',toolHeight);
                $('.appContainer').addClass('fixedPane').attr('data-top',appTop);
                $('.appContainerDummy').css('height',$('.appContainer').outerHeight()).show();
            }else if(prevAppTop!=undefined && st<prevAppTop){
                $('.appContainer').removeClass('fixedPane').removeAttr('data-top').css({'top':'','right':'','max-height':'','width':''});
                $('.appContainerDummy').hide();
            }
            $('.appContainer').css({'right':'','left':''});
            $('.appContainer.fixedPane').each(function(){
                var rightPos=$(window).width()-$('#page-area').width()+$(window).scrollLeft();
                if($(window).width()>$('#page-area').width()){
                    rightPos=rightPos/2;
                }
                $(this).css({
                    'right': rightPos,
                    'max-height':h-toolHeight,
                    'width':$(this).closest('#rightPane').width()
            ...
            if($('#footer-area').is(':visible')){
                var footTop=$('#footer-area').offset().top*1;
                var footHeight=$('#footer-area').outerHeight()*1;
                var prevFootTop=$('#footer-area').attr('data-top');
                if(prevFootTop)prevFootTop=prevFootTop*1;
                if(st+h>=footTop+footHeight && typeof prevFootTop=='undefined'){
                    $('#footer-area').addClass('fixedPane').attr('data-top',$('#centerInner').height()+$('#header-area').height()+toolHeight);
                    $('#footer-area-dummy').css('height',footHeight).show();
                }else if(prevFootTop!=undefined && parseInt(st+h)<eval(parseInt($('#centerInner').height()+$('#header-area').height()+toolHeight)+parseInt(footHeight))){
                    $('#footer-area').removeClass('fixedPane').removeAttr('data-top');
                    $('#footer-area-dummy').hide();
                }
            }

	     if($('.js_workspace_content:visible')[0]){
                if($('#articleToolbar').hasClass('fixedPane')){
                    var wsTop=$('#articleToolbar').outerHeight();
                }else{
                    var wsTop=$('#articleToolbar').outerHeight()+$('#header-area').height();
                }
                $('.js_workspace_content').css('top',wsTop);
            }
 
        if($('.js_workspace_content').is(':visible'))rightSidePane.resetWorkspace();
        if($('#pdfModalWindowMsgBox').is(':visible'))pdfCite.alignSuggestedArticleBox();
        $(window).on("scroll.viewHandler", page.viewHandler );
        page.adjustRightPane();
      },
      adjustRightPane:function(){
        ...
        $('body').css('max-width','');
        if (1366 <= pgLayout.vpWidth) {
            pgLayout.vpWidth=1306;
            pgLayout.lcWidth = 260;
            pgLayout.rcWidth = pgLayout.vpWidth - pgLayout.lcWidth - pgLayout.ccWidth-1;
        }else if (1280-$.scrollBarWidth() <= pgLayout.vpWidth) {
            if(pgLayout.vpWidth>1306)pgLayout.vpWidth=1306;
            pgLayout.lcWidth = 260;
            pgLayout.rcWidth = pgLayout.vpWidth - pgLayout.lcWidth - pgLayout.ccWidth-1;
        }else if (1024-$.scrollBarWidth() <= pgLayout.vpWidth) {
            if(pgLayout.vpWidth<1240){
                pgLayout.lcWidth = 28;
                if(pgLayout.vpWidth<1008){
                    pgLayout.vpWidth=1008;
                    $('body').css('max-width',pgLayout.vpWidth);
                }else if(pgLayout.vpWidth<1074){
                    $('body').css('max-width',pgLayout.vpWidth);
                }else{
                    pgLayout.vpWidth=1074;
                    $('body').css('max-width',pgLayout.vpWidth);
                }
            }else{
                pgLayout.lcWidth = 260;
        ...
        pgLayout.lcWidth=260;
        if($(window).height()>$('body').height()){pgLayout.rcWidth-=2;}
        $('#rightPane,.appContainer.fixedPane').css('width',pgLayout.rcWidth);
      },
      showLeftAd:function() {
      ...
      doHighlighting:function(ev) {
        ...
        $('html,body').moveTo(linkId.substring(1));
        ev.preventDefault();
      },
      ...
      searchWithinErrorMsg:function(msg) {
        ...
        $('#searchWithin > div').prepend('<div class="errorMsg"><div class="msg"><div class="alertIcon"></div><span>' + msg + '</span></div></div>').addClass('bookError');
        $('#outline').addClass('bookError');
        $('#outline').css('top',24+$('#searchWithin').outerHeight());
      },
      searchWithinErrorMsgClear:function() {
        $('#searchWithin, #outline').removeClass('bookError');
        $('#searchWithin .errorMsg').remove();
        $('#outline.book').css('top',24+$('#searchWithin').outerHeight());
      },
      ...
      moveToSection: function(e) {
          ...
          selID=selID[1];
          $('html,body').moveTo(selID, e.data);
          e.preventDefault();
          e.stopPropagation();
      }, 
      addJS: function() {
        if(SDU.isIE() && SDU.isIEless(7)) {return;}
...
//        page.addDynamics();
        ...
        if (Fragment.isAvail()) {
           ...
           $("#rightPane").on("click", ".viewInArt", {z:'rightPane'}, Outline.findTargetFragment);
           $('#centerPane').on('click', '.FCANote', Fragment.handleAipClicks);
        } else {
           $("#outline").on("click", ".tocLink", Outline.moveToSection);
           $("#rightPane").on("click", ".viewInArt", page.moveToSection);
           $('#centerPane').on('click', '.FCANote', {focus: false}, page.moveToSection);
        }

	$('#centerPane').on('click', '.grantnum' , {focus: false}, page.moveToSection);
        ...
        if(!SDM.crawlerAvail) { resolveReferences('centerPane'); }
    ...
    if((!SDM.crawlerAvail) &&(SDM.citedByScAvail==true)) getcitedByScopusCount();
    page.startOutline();
...
$.fn.outline = function(oObj, opts, tocJson) {
  ...
  return this.each(function() {
    ...
    oObj.prepend ('<div id="olGraphCbBox"></div>');
    
    $('#olGraphCbBox').append($('.outlineMsg'));
    $('#olGraphCbBox').append('<ul id="itemOutline"></ul>');
    var $iOutline = $('#itemOutline');
    oObj.append('<div id="fig" role="navigation" aria-labelledby="fig_title">'
                        +'<hr><h2 id="fig_title">Figures and tables</h2>'
                        +'<ol class="figures " role="tree">'
                        +'</ol></div>');
    var level = "";
...
//$section = $('<li class="" style="margin-right:16px;"></li>');
  ...
  if(sec.sT != undefined){
    html += '<a onclick="SD_UTIL.sendUserKeyEvent(\'displayTOCSection\', \'article\', \'leftPane\', '+SDM.keOriginContentFamily +'); return $(\'html,body\').moveTo(\''+ sec.sID +'\')"  href=\'#'+sec.sID+'\' >'+sec.sT+'</a>';
  }
...
var Outline = {
  ...
  init:function() {
    ...
    $('#leftPane').on('click','#leftButton',function(e) {Outline.sidebarClick(e)});
    $('body').on('click','#outlineContainer .closeBt',function(e){
        Outline.toggleOutlinePopup(true);
    });
  },
  buildOutlineJsonSide:function(cfg, outlineObj) {
    var $iOutline = $('#itemOutline');
    var parentItem='';
    var $figureOutline = $('#fig .figures');
      if (outlineObj!=null && outlineObj.TOC != undefined && outlineObj.TOC.length>1) {
        $('#olGraphCbBox').prepend('<div class="olHead"><h2 id="toc_title">Article outline</h2>'
                            + '<label><input id="outlineGraphicsCheckBox" type="checkBox">Show full outline</label></div>');
            ...
            $(h2Sec.faID).each(function(i, floatanc) {
              if(floatanc.fT == 'fig'){
                ...
                if(floatanc.fI.length>0){
                  $figureOutline.append(Outline.addItemFig(floatanc.fID, floatanc.fL, imageURL, 'h2', floatanc.fIh,parentItem));
                }
              }
              else if(floatanc.fT == 'tbl'){
                $figureOutline.append(Outline.addItemTbl(floatanc.fID, floatanc.fL, 'h2',parentItem));
              }
              else if(floatanc.fT == 'ecomp'){
                $figureOutline.append(Outline.addItemMMC(floatanc.fID, floatanc.fL, floatanc.floatEXT, 'h2',parentItem));
              }
            })
          }
            ...
            $(h2Sec.cT).each(function(i, h3Sec) {
              ...
              if(h3Sec.faID != undefined && h3Sec.faID.length > 0){
                $(h3Sec.faID).each(function(i, floatanc){
                  if(floatanc.fT == 'fig'){
                    var imageURL = SDM.imageUrlPrefix + outlineObj.EID + '-' + floatanc.fI;
                    if (floatanc.fI.length>0){
                      $figureOutline.append(Outline.addItemFig(floatanc.fID, floatanc.fL, imageURL, 'h3', floatanc.fIh,parentItem));
                    }
                  }
                  else if(floatanc.fT == 'tbl'){
                    $figureOutline.append(Outline.addItemTbl(floatanc.fID, floatanc.fL, 'h3',parentItem));
                  }
                  else if(floatanc.fT == 'ecomp'){
                    $figureOutline.append(Outline.addItemMMC(floatanc.fID, floatanc.fL, floatanc.floatEXT, 'h3',parentItem));
                  }
                })
              }
              if(h3Sec.cT != undefined && h3Sec.cT.length > 0){
                $(h3Sec.cT).each(function(i, h4Sec) {
                  ...
                  parentItem=h4Sec.sID;
                }
                  if(h4Sec.faID != undefined && h4Sec.faID.length > 0){
                    $(h4Sec.faID).each(function(i, floatanc){
                      if(floatanc.fT == 'fig'){
                        var imageURL = SDM.imageUrlPrefix + outlineObj.EID + '-' + floatanc.fI;
                        $figureOutline.append(Outline.addItemFig(floatanc.fID, floatanc.fL, imageURL, 'h4', floatanc.fIh,parentItem));
                      }
                      else if(floatanc.fT == 'tbl'){
                        $figureOutline.append(Outline.addItemTbl(floatanc.fID, floatanc.fL, 'h4'));
                      }
                      else if(floatanc.fT == 'ecomp'){
                        $figureOutline.append(Outline.addItemMMC(floatanc.fID, floatanc.fL, floatanc.floatEXT, 'h4',parentItem));
                      }
                    })
                  }
                })
      ...
      if($figureOutline.children().size()==0){
        $('#fig').remove();
      }
      Outline.postBuild(cfg);
  },
      ...
      $(cfg.selector).each(function() {
        if($(this).parents('.textboxBody').length>0) {}
          ...
          if($(this).is("h2")||$(this).is("h3")||$(this).is("h4")) {
            if($(this).is("h3")||$(this).is("h4"))$('#olGraphCbBox label').show();
            ...
            parentItem=$(this).attr('id');
          }
          else {
            if($(this).hasClass('table')==true) {
              var label = $(this).attr('data-label');
              if(label==undefined) label='';
              $figureOutline.append(Outline.addItemTbl($(this).attr('id'), label, level,parentItem));
            }
              ...
              if(!$(this).children('div').attr('id')){
                $figureOutline.append(Outline.addItemFig($(this).attr('id'), label, image, level, thumbHeight,parentItem));
              }
            }
            else if($(this).hasClass('ecomponent')==true) {
              var label = $(this).attr('data-label');
              if(label==undefined) label='';
              var ext = $(this).find('dd:first').attr('data-ext');
              $figureOutline.append(Outline.addItemMMC($(this).attr('id'), label, ext, level,parentItem));
            }
            else {}
          }
        }
      });
      if ($('#itemOutline').text() != '')$('#leftPane .outlineMsg').remove();
    }
    else {
      if(SDM.pm.contentType!='BK' && !SDM.isMRWMODArticle) {
        $('#outline').css('top', 0);
  ...
  postBuild:function(cfg) {
    ...
    var leftAdState = getCookie('ad_left');
    if(!leftAdState){
        oObj.append('<div id="articleLeftAd" class="adBlock"></div>');
        Outline.showLeftAd();
    }
    if(cfg.searchWithin && !$('#searchWithin')[0]) {
      oObj.parent().prepend('<div id="searchWithin" class="textEntry" role="Search" aria-label="Book">'
          + '<div><form name="searchWithinForm" action="javascript:checkSearchWithin();ArticlePage.searchWithinSubmit();">'
          + '<input type="text" value="Search this book" title="Search this book" size="24" maxlength="450"></input></form>'
          + '<button class="clearWithin" title="Clear book search"><div class="clearXHide"></div></button>'
          + '<button class="submit" title="Search this book"></button></div>'
          + '</div>');
    ...
    if(SDM.isMRWMODArticle && SDM.ldrAvail && !$('#lpTabs')[0]) {$('.outlineMsg').hide();mrwLeftPane.init();}
    //Setup the sync between rightPane scroll and outline highlight
    if(cfg.focusEffect==true) {
      $('#itemOutline').syncTo(cfg);
    }
    ...
    oObj.scroll();
    oObj.children().not('#srcOutline').outerWidth(pgLayout.lcWidth-$.scrollBarWidth());
    Outline.setScroll();
  },
  setScroll:function(){
    $(Outline.oObj).on('mousewheel DOMMouseScroll', function(e) {
        var scrollTo = null;
  ...
  addItemMMC: function(hashId, lbl, ext, hStr,parentElm) {
    ...
    var html;
    if(SDM.pageType == 'article_full'){
      html = "<li><div id='ol_" + hashId + "' class='tocLink io item mmc' data-parent='"+parentElm+"'>"
           + "<a class=\"tocLink olIcon S_C_tocLink" + ecn + "\" href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection', 'article', 'leftPane', '" + SDM.keOriginContentFamily + "'); \"> <div class=\"olIcon " + ecn + "\"></div>"
           + lbl + "</a></div></li>";
    }
    else if (SDM.pageType == 'article_abs') {
      html = "<li><div id='ol_" + hashId + "' class='io item mmc' data-parent='"+parentElm+"'>"
           + "<a class=\"tocLink olIcon S_C_tocLink" + ecn + "  cLink\"  queryStr='" + SDM.urlTOCLinkQueryStr + "' href='" + SDM.urlTOCLink + "'\> <div class=\"olIcon " + ecn + "\"></div>"
           + lbl
           + "</a></div></li>";
    }
    return html;
  },  //Outline.addItemMMC
  addItemTbl: function(hashId, lbl, hStr,parentElm) {
    var html;
    if(SDM.pageType == 'article_full'){
     html = "<li><div id='ol_" + hashId + "' class='io item tbl' data-parent='"+parentElm+"'>"
             + "<a class=\"tocLink olIcon olIconTbl S_C_tocLinkTbl\" href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection', 'article', 'leftPane', '" + SDM.keOriginContentFamily + "'); \"><div class=\"olIcon olIconTbl\"></div>" 
             + lbl + "</a></div></li>";
    }
    else if (SDM.pageType == 'article_abs') {
    html = "<li><div id='ol_" + hashId + "' class='io item tbl' data-parent='"+parentElm+"'>"
             + "<a class=\"olIcon olIconTbl cLink S_C_tocLinkTbl\"  queryStr='" + SDM.urlTOCLinkQueryStr + "' href='" + SDM.urlTOCLink + "'\><div class=\"olIcon olIconTbl\"></div>" 
             + lbl + "</a></div></li>";
    }
    return html;
  },  //Outline.addOutlineItemTbl
  addItemFig: function(hashId, lbl, img, hStr, thumbHeight,parentElm) {
    var html;
    if (SDM.pageType == 'article_full') {
      html = "<li><div id='ol_" + hashId + "' class='io item fig' data-parent='"+parentElm+"'>"
             + "<a class='tocLink S_C_tocLinkFig' href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection','article','leftPane', '" + SDM.keOriginContentFamily + "');\">"
      ...
      html = "<li><div id='ol_" + hashId + "' class='io item fig' data-parent='"+parentElm+"'>"
           + "<a class=\"olIcon olIconFig  cLink S_C_tocLinkFig\"  queryStr='" + SDM.urlTOCLinkQueryStr + "' href='" + SDM.urlTOCLink + "'\>" 
  ...
  addItemSection: function(hashId, lbl, hStr) {
    var html = "<li class='li_"+hStr+"'><div id='ol_" + hashId + "' class='io item " + hStr + "sec'>"
             + "<a class='tocLink S_C_tocLinkSec'href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection', 'article', 'leftPane', '" + SDM.keOriginContentFamily + "');\">"
             + lbl + "<br>"
             + "</a></div></li>";
    return html;
  ...
  addItemOutline: function(hashId, lbl, hStr) {
    var html = "<li class='li_"+hStr+"'><div id='ol_" + hashId + "' class='io item " + hStr + "sec'>"
             + "<a class='cLink S_C_tocLinkOutline' queryStr='" + SDM.urlTOCLinkQueryStr + "&_hashID=" + hashId+"' href='" + SDM.urlTOCLink + "'\>"
             + lbl + "<br>"
             + "</a></div></li>";
    return html;
  }, //Outline.addOutlineItemSection
  sidebarClick: function(e) {
    ...
    e.preventDefault();
    e.stopPropagation();
    Outline.toggleOutlinePopup();
    SD_UTIL.sendUserKeyEvent('openLeftPane', 'article', 'leftPane', SDM.keOriginContentFamily);
    return false;
  },
  toggleOutlinePopup: function(e) {
    if($('#outlineContainer').is(':visible')) {
        $('#outlineContainer').removeClass('on').css({'top':0});
        $('#leftButton .outline-toggle').attr('title','Open to view article outline');
        $('#leftButton .outline-toggle').attr('alt','Open to view article outline');
        $('#leftButton .outline-toggle').attr('aria-expanded','false');
    }else if(!e){
        $('#outlineContainer').addClass('on');
        $('#leftButton .outline-toggle').attr('title','Close the article outline');
        $('#leftButton .outline-toggle').attr('alt','Close the article outline');
        $('#leftButton .outline-toggle').attr('aria-expanded','true');
        if($('#searchWithin').is(':visible'))$('#outline.book').css('top',20+$('#searchWithin').outerHeight());
  ...
  toggleGraphics: function(force) {
    ...
    var $toc = $('#itemOutline', '#outline');
    if($('#outlineGraphicsCheckBox')[0])$toc.attr('aria-expanded', SDM.outlineGraphics).toggleClass('thumbnails', $('#outlineGraphicsCheckBox')[0].checked)
    $('#outlineGraphicsCheckBox').attr('aria-expanded', SDM.outlineGraphics);
  ...
  findTargetFragment: function(e) {
    ...
    if (!$(obj)[0]) {
      ...
      if (Fragment.crossRefMap) {
        //var base = targetFragNum-2;
        //if(base <= 1) base = 1;
        Fragment.loadTarget(selID, {preBuffer:2,postBuffer:2,f:function() {Outline.findTargetFragment(e);}});
        //MoreInformation.fragLoad( {c:base,
        //                           s:base,
        //                           l:5,
        //                           f:function() {
        //                             Outline.findTargetFragment(e);
        //                           }
        //                           });
      }
    }
    else {
      var testObj = $('#frag_'+ (targetFragNum+1));
      if (!$(testObj)[0]) {
      ...
      Fragment.scrollFlag = false;
      $('html,body').moveTo(selID, {  cbFunc: function() {
        Fragment.scrollFlag=true;
      }});
    } 
...
$(document).ready(function() {
    ...
    if(SDU.isIE7() && SDM.BROWSER_SPECIFIC_USR_MSG)$('#ieWarningMsg').ieWarning();
    if(SDM.SD_System_Msg)prodMsg.init();
        //loadBreadCrumbs();
    setTimeout(function(){if(SDM.secId!=''&& SDM.secId!= undefined ){
          $('html,body').moveTo (SDM.secId);
    }},1500);
    exportCite.init();
    exportCite.Mendeley();
});
...
$.fn.lazyLoadImages = function(opts) {
  ...
  function scrollAction() {
    ...
    for(i=0;i<totalCount && loopBool==true ;i++){
      ...
      if( Math.abs($(window).scrollTop()+$(window).height() - $(obj).positionA('#centerContent').top+$(obj).height()) < 
                                                                     $(window).height() + cfg.preLoadLine
       || Math.abs($(window).scrollTop() - $(obj).positionA('#centerContent').top)  < $(window).height() + cfg.preLoadLine) {
        var imgObj = $(obj);
        if(imgObj.attr('data-loaded')!='true' ) {
          ...
          if (dataGabsImgEID != undefined && dataGabsImgEID != '') {
              imgObj.attr('src', SDM.imageUrlPrefix + imgObj.attr('data-gabsEID')).attr('data-loaded', 'true');
          }else if (dataFullImgEID != undefined && dataFullImgEID != '') {
            ...
            if(imgObj.closest('.formula')[0] && imgObj.width()>=595){
                if(imgObj.closest('.formula').find('div.label')[0]){
                    imgObj.closest('.formula').find('.mathmlsrc').addClass('scrollOn');
                }else if(imgObj.width()>616){
                    imgObj.closest('.formula').find('.mathmlsrc').addClass('scrollOn');
                }
            }
          ...
          if($(window).scrollTop()+$(window).height() - $(obj).positionA('#centerContent').top+$(obj).height() < 
             $(window).height() + cfg.preLoadLine && scrollDown){ 
              if (!mathRenderer.opt.renderOn) { 
	        loopBool=false;
              }
          }
          if($(window).scrollTop() - $(obj).positionA('#centerContent').top  <  $(window).height() + cfg.preLoadLine && scrollUp){
              if (!mathRenderer.opt.renderOn) {
                loopBool=false;
              }
          }
        ...
        if( Math.abs(myDims - outlineTop) < Math.abs(outlineBottom + cfg.preLoadLine) 
         || Math.abs(outlineTop - myTop)  < Math.abs(outlineHeight + cfg.preLoadLine)) {
          ...
          if(imgObj.attr('data-loaded')!='true' ) {
            ...
            $(this).removeClass('greyImg smlImgLazyJSB');
          } 
...
$.fn.lazyLoad = function( cfg ){
  ...
  var plugin = {
    ...
    scrollSrc: cfg.scrollSrc?cfg.scrollSrc:window,
           ...
           $(".citedBy_").each(function(){
              if(myXabsCounts[this.id]) {
                 if( this.innerHTML.match('Cited By in Scopus') == null) {
                  ...
                  $(this).closest('div.boxLink').before('&nbsp;|&nbsp;');
...
function delayedResolveRefs(s,c,d) {
  setTimeout(function() {resolveRefs(s, c);}, d);
}

function resolveReferences(fId) {
  $refs = $('#' + fId + ' .refPlaceHolder');
  var rl = $refs.length-1;
  if(rl>=0) {
    if(rl > SDM.pf.ref.bs) {
      var c = Math.ceil(rl/SDM.pf.ref.bs);
      var m = rl%SDM.pf.ref.bs;
      var st = $($refs[0]).attr('id').substring(8)*1;
      var ct = SDM.pf.ref.bs;
      for(var i=1;i<=c;i++) {
        if(i==c && m!=0) {
          ct = m;
        }
        if(Fragment.isAvail())
          ...
          delayedResolveRefs(st,ct, SDM.pf.ref.fd+(i*10));
        st = st + SDM.pf.ref.bs;
      }
    }
    else {
      var start = $($refs[0]).attr('id').substring(8);
      var count = rl;
      ...
      .success(function(res) {
        ...
        if (refMap != null) {
          ...
          if(citedBySCEids != null && citedBySCEids != ""){
            updateCitedByCounts(citedBySCEids,false,start,count);
          }
        }
      })
      .error(function() {
...
var EbookTOC = {
  ...
  init: function() {
    ...
    if(EbookTOC.tocObj==null) {
      $.getJSON(eBookTOCURL, function(res) {
        ...
        if(!$('#lpTabs')[0])EbookTOC.load(res);
      });
    }
    else {
      if(!$('#lpTabs')[0])EbookTOC.load(EbookTOC.tocObj);
    }
  ...
  load:function(res) {
    ...
    $('#outlineContainer').prepend('<ul id="lpTabs">'
    ...
    $('#outline, #leftPane, #outlineContainer').addClass('book');
  ...
  showItemTab:function(e) {
    ...
    $('#srcOutline').hide();
    $('#searchWithin').hide();
    $('#olGraphCbBox,#fig').show();
    if($('#outlineGraphicsCheckBox').prop('checked')==true) {
     Outline.toggleGraphics('show');
    }
    $('#outline').scrollTop(0);
    $('#outline').css('top', '27px');
  },
  ...
  loadsection = new $.ajax( {
      ...
      success: function(res) {
        ...
        if($('#'+section).position()!=null) {
          $('html,body').moveTo(section);
        }
        var secTitle=$('#sectitle');    
        $('#ol_sectitle').html("<a onclick=\"return $('html,body').moveTo('sectitle')\"  href='"+secTitle.attr('id')+"'>"+secTitle.html()+"<br></a>");
      }
  });
}
...
function loadSectionFrag(baseUrl,section) {
  var endPoint = baseUrl+section;
  var div1 = doc.getById("indexSection");
  $(window).unbind('scroll', Fragment.scroll);
...
var ArticleToolbar = {
  ...
  toggleOptions:function(e){
    if( $("#moreOptionsMenu").is(":visible") ){
      ...
      $("#moreOptionsButton div").addClass("upSM_" + SD_UTIL.getProdColor() );
      ...
      $(window).bind('scroll',ArticleToolbar.closeOptions);  
    }
    exportPopupClose(e);
    SD_UTIL.killEvent( e );
  },
  closeOptions:function(e){
    ...
    $("#moreOptionsButton div").removeClass( "upSM_" + SD_UTIL.getProdColor() );
      if(SDU.isIE7() || SDM.adReqOptions==false) {
    ...
    $(window).unbind('scroll',ArticleToolbar.closeOptions );                
    }
  }
};
...
var LoginBox = {
  ...
  flipLogin: function (e,button){
    ...
    if(button == null){
      if (t.display=="none"){
        ...
        j.className = 'minus';
        $('#username').focus();
        $('#loginPlusScript').attr('aria-expanded','true');
        $(document).on('keyup',LoginBox.handleEscKey);
      }
      else{
        t.display = 'none';
        j.className = 'plus';
        $('#loginPlusScript').attr('aria-expanded','false');
        $(document).off('keyup',LoginBox.handleEscKey);
      }
    }
      ...
      if (t.display=="none" ){
        ...
        $('#userPlus').attr('aria-expanded','true');
        $(document).on('keyup',LoginBox.handleEscKey);
      }
      else{
        t.display = 'none';
        v.className = 'userPlus';
        $('#userPlus').attr('aria-expanded','false');
        $(document).off('keyup',LoginBox.handleEscKey);
      }
    }
    else{
  ...
  handleEscKey:function(e){ 
    if(e.keyCode == 27) 
    {
        if($('#loginBox,#userBox').is(':visible'))
        {
            $('#loginBox,#userBox').hide();
			$('#loginPlusScript').addClass('plus').removeClass('minus');
            $('#userPlus').addClass('userPlus').removeClass('userMinus');
        }
    }
  },
  setTabs:function(){
    $('#header').off('keydown','#loginBox :tabbable:last,#userBox :tabbable:last')
                .on('keydown','#loginBox :tabbable:last,#userBox :tabbable:last', function(e) {
        if (e.which == 9 && !e.shiftKey) {
            e.preventDefault();
            $(this).closest('#loginBox,#userBox').find(':tabbable').eq(0).focus();
        } 
    });
     $('#header').off('keydown','#loginBox :tabbable:first,#userBox :tabbable:first')
                .on('keydown','#loginBox :tabbable:first,#userBox :tabbable:first', function(e) {
        if (e.shiftKey && e.which == 9) {
            e.preventDefault();
            $(this).closest('#loginBox,#userBox').find(':tabbable').eq(-1).focus();
        } 
    });
  }
}//LoginBox
...
var CanonicalLink = {
  init: function() {
    $("body").on("click",".cLink:not('.pdfRightPaneLink')", function(event) {
      var t = $(this);
  ...
  if(zone == "subjectBrowse")
  {
     ...
     createCookie ("MRW_MODULE_STATE_COOKIE", cv);
      ...
      if (qs) {
        ...
        .addTag(qs.getParameter("_piiSuggestedFrom"), 'sf')
        .addTag(qs.getParameter("gatewayReq"), 'gr')
        .addTag(qs.getParameter("authShareStatus"), 'au');
...
var MobileLink = {
  init: function() {
    $("body").delegate(".mLink", "click", function(event) {
      var t = $(this);
      overrideSitePref(t.attr("site"), t.attr("ckNm"), t.attr("lnkOrg"), t.attr("lnkZn"), event);
    });
  }
} //MobileLink
$(document).ready(MobileLink.init);

function overrideSitePref(sitePref,cookieName,orig,zone){
  if (!sitePref){sitePref="";}
  if (!cookieName){cookieName="Site";}
  createSimpleCookie(cookieName,sitePref);
  var cv='';
  if (orig&&(''!==orig)){cv=cv.addTag(orig,'org');}
  if (zone&&(''!==zone)){cv=cv.addTag(zone,'z');}
  if (''!==cv){
    cv=cv.addTag(getDateTime(), 'rdt');
    createCookie("SD_ART_LINK_STATE",cv);
  }
  var pgRLForm=document.getElementById('pgReload');
  if (pgRLForm){
    pgRLForm.action=window.location.href;
    pgRLForm.submit();
  }else{
...
function createSimpleCookie(cookieName, value) {
  var finalCookie=cookieName+"="+value+";path=/";
...
var pdfCite = {
  ...
  openPDF:function(url, event) {
    ...
    // Bug in IE, PDF is displayed as blank page while using window.open()
    if ($.browser.msie) {
       pdfWin=window.open('','_blank','width='+newWidth+',height='+newHeight+',location=yes,scrollbars=yes,resizable=no,directories=no,toolbar=yes,menubar=yes,status=yes,left=50,top=50');
       pdfWin.location.href (url);
    } else {
       pdfWin=window.open(url,'_blank','width='+newWidth+',height='+newHeight+',location=yes,scrollbars=yes,resizable=no,directories=no,toolbar=yes,menubar=yes,status=yes,left=50,top=50');
    }
    pdfWin.focus();
  ...
  alignSuggestedArticleBox:function() {
    ...
    var top = (winHeight - msgBox.outerHeight())/2;
    var left = (winWidth - msgBox.outerWidth())/2;
  ...
  var helpURL = SDM.helpUrlDomain+ "/sdhelp_CSH.htm#citing_art_lrn_more.htm";
    ...
    if (obj != null) {
      ...
      $.each (obj.citation, function(i, item){
         ...
         if (item.OASTATUS =="1" || item.OPENARCHIVESTATUS =="1") {
           artList +='artTitleOA'; 
...
var EComponent = {
  ...
  init: function(fragId) {
    ...
    $(this.scope + '.MMCvVIDEO').each(function() {
      var mmcid = $(this).attr('mmcimageurl');
      if (mmcid != '') {
        $(this).attr('mmcimageurl', SDM.doiUrlPrfx + "/cache/MiamiImageURL/" + mmcid);
        var mmcflashvar = $(this).attr('mmcvflashvars');
        if (mmcflashvar != '') {
          $(this).attr('mmcvflashvars', mmcflashvar + "&image=" + SDM.doiUrlPrfx + "/cache/MiamiImageURL/" + mmcid);
        }
      }
      swfobject.embedSWF(
...
function textBoxCE(textObj,imptr) {
  if(textObj==''){
    textObj = $(imptr).attr('data-img');
  }
  var child=doc.getById(textObj);
  if(child.style.display!="block") {
    child.style.display="block";
    imptr.src="/scidirimg/minus.gif";
  }
  else{
    child.style.display="none";
    imptr.src="/scidirimg/plus.gif";
  }
}
...
function requestHeightAdjust(id, h) {
  if(h>50 && !$('#' + id).find('.adText')[0]){
    $('#' + id).prepend('<div class="divider"></div><div class="adText">ADVERTISEMENT<div class="closeBt"><a href="#" title="Close Advertisement">&nbsp;</a></div></div>');
  }
  if(h>50){
      $('#' + id + ' iframe').height(h);
      $('#' + id).height(h+65);
...
var rightSidePane={ citedType:false,
  sendRightPaneKeyEvents: function(a, o, z) {
    $.get(SDM.userActionURL+'/'+o+'/'+z+'/'+a);
  },
  initAccordion: function () {
    rightSidePane.getRelatedCitingPanel();
    var rightAdState = getCookie('ad_right');
    if(!rightAdState)rightSidePane.showRightAd($('#rightInner .innerPadding'));
  },
  ...
  getRelatedCitingPanel:function(obj){
    ...
    $('#rightPane').off('click.related').on('click.related', '.related .toggle', function(e) {
        var expanded = 'true' == $(this).attr('aria-expanded') ? 'false': 'true';
        var artContainer=$(e.currentTarget).closest(".entry").toggleClass('closed', 'true' != expanded);
        var artType=artContainer.attr('id');
        $(this).attr('aria-expanded', expanded);
        if(expanded=="true" && !artContainer.hasClass('rendered')){
            if(artType=="special_articles" && SDM.isSpecialIssue){
                var specIssURL=SDM.urlPrefix + '/spliss/rslts/' + SDM.hubEid + '/' + SDM.pm.pii;
                var getArticleList=function(res){
                    if(res!='' && res.HITCOUNT>0){
                        rightSidePane.buildArticleList(res, "specIssueList", true);
                    }else{
                        $('#specIssueList .note').show();
                    }
                };
                $.getJSON (specIssURL, getArticleList).fail(function(){$('#specIssueList .note').show();});
                artContainer.addClass('rendered');
            }
            if(artType=="ref_mod_articles" && SDM.isMRWMODArticle){
                var refCounter = 0;
                var trd = 200;
                ///////////
                var refCheck = function() {
                    ...
                    if(typeof conceptID!='undefined' || refCounter>10) {
                        if(typeof conceptID!='undefined'){
                            conceptID=conceptID.substring(conceptID.indexOf('Concept-'),conceptID.indexOf('?'));
                            $('#mrwTopicTitle a.toggle').html('Reference articles on "'+ $(activeNode).text().replace(/^\s+|\s+$/g,'')+'"');
                            ...
                            var getArticleList=function(res){
                                if(res.TYPE!=undefined && res.TYPE!='' && res.citation.length>0){
                                    ...
                                    $('#mrwTopicTitle').parent().show();
                                }else{
                                    $('#mrwTopicTitle').parent().hide();
                                    $('.related:visible .toggle:first').trigger('click').closest('.entry').addClass('firstBlk');
                                }
                            };
            ...
            if(artType=="recommend_related_articles" && SDM.mlktAvail){
                var relatedArtURL=SDM.urlPrefix +'/mlkt/rslts/'+SDM.pm.pii+'/Art'
                var getArticleList=function(res){
                    if(res!='' && res.HITCOUNT>0){
                        rightSidePane.buildArticleList(res, "relArtList");
                    }else{
                        $('#relArtList .note').show();
                    }
                };
                $.getJSON (relatedArtURL, getArticleList).fail(function(){$('#relArtList .note').show();});
                artContainer.addClass('rendered');
            }   
            if(artType=="related_ref_works_articles" && SDM.relatedRefAvail) {
                var relatedRefURL=SDM.urlPrefix +'/mlkt/rslts/'+SDM.pm.pii+'/Ref';
                var getArticleList=function(res){
                    if(res!='' && res.HITCOUNT>0){
                        rightSidePane.buildArticleList(res, "relRefList");
                    }else{
                        $('#relRefList .note').show();
                    }
                };
                $.getJSON (relatedRefURL, getArticleList).fail(function(){$('#relRefList .note').show();});
                artContainer.addClass('rendered');
            }
            if(artType=="cited_by_articles" && SDM.citedByScAvail) {
                rightSidePane.citedType=true;			
                var citedByURL=SDM.urlPrefix +'/citedby/rslts/'+SDM.pm.pii;
                var getArticleList=function(res){
                    if(res!='' && res.HITCOUNT>0){
                        rightSidePane.buildArticleList(res, "citedByList");
                    }else{
                        $('#citedByList .note').show();
                    }
                };
                $.getJSON (citedByURL, getArticleList).fail(function(){$('#citedByList .note').show();});
                artContainer.addClass('rendered');
            }
        }
        $('#rightOuter').css('height','');
        if(typeof ArticlePage != 'undefined')ArticlePage.viewHandler();
        e.stopPropagation();
        e.preventDefault();
    });
  ...
  toggleMoreInfo:function(e){
    var obj = $(e.target);
    var showMoreUrl = $(obj).attr ("data-url");
    var moreInfo = $(obj).closest('ol.articles').find("li.showMore");
  ...
  buildArticleList:function(res, targetId, isSpecialIssue) {
    ...
    if (res) {
      ...
      if(cacheType == '9') {
        var count=$(".titleEditors").length;
        var specIssueTitle = $(".specIssueTitle").text();
        var specIssueUrl = $(".publicationHead p.volIssue").find("a").attr("href");
        artList += '<li class="specIssTitle"><a class="cLink" queryStr="?&zone=rightPane&panel=biblioInfo" href="' + specIssueUrl + '">' + specIssueTitle + '</a>';
        for(var i=0;i<count;i++){
           var titleEditors = $(".titleEditors:eq("+i+")").text();
           artList += '<li class="editor">'+ titleEditors + '</li>';
        } 
        artList += '<li><h3>Other articles from this special issue</h3></li>';
      }
      if ($(res.citation).length < 1) {
        $('#'+targetId).closest('.entry').find('.note').show();
        return;
      }
      $.each (res.citation, function(i, item){
          var key = item.PII?item.PII:item.SCEID;
          var showMoreUrl = SDM.urlPrefix + '/more/' + key + '/' + cacheType; 
          if(rightSidePane.citedType==true && (item.ARTURL.indexOf("method=outwardLink") != -1)){
              artList += '<li><ol class="articles"><li class="artTitle"><a target="_blank" title="' + item.ARTTITLE + '" href="' + item.ARTURL + '"';
          }else{
              artList += '<li><ol class="articles"><li class="artTitle"><a title="' + item.ARTTITLE + '" href="' + item.ARTURL + '"';
          }
          if (res.QUERYSTRING) {
              artList += ' class="cLink S_C_artTitle" queryStr="' + res.QUERYSTRING + '"';
          }
          artList += '>' + item.ARTTITLE + '</a></li>';
          if (isSpecialIssue == true) {
            artList += '<li class="authors">' + item.AUTHORS;
          } else {
            artList += '<li class="srcTitle">' + item.PUBYR;
            if (item.PUBYR) { artList += ', '; }
            artList += getCombinedSrcTitle(item);
          }
          artList += '<div class="moreInfo"><a data-url="' + showMoreUrl +  '" class="toggleMoreInfo">more</a></div></li>'
          artList += '<li class="showMore hidden">'
                  +  '<div class="moreInfoContent">'
                  +  '<ol class="details"><li>' + item.AUTHORS + '</li><li class="title">' + item.ARTTITLE + '</li>';
	    ...
	    if(item.PDF_NOFOLLOW == "true")artList += 'rel="nofollow" ';
            artList += 'title="Download PDF"  target="_blank" class="cLink pdfRightPaneLink" href="';
            artList += item.PDFURL + '"';
             if(item.PDFURL) {
          ...
          if (cacheType == '5' && item.VIEW_RECORD_URL) {
              artList += '<ul><li><a target="_blank" href="' + item.VIEW_RECORD_URL + '">View details in Scopus</a></li><ul>';
          }
        ...
        if (Fragment.isAvail()) {
            if (isAuthorReq) {
                ...
                if ($(affObj)[0] && !$($(affObj).attr('href'))[0]) {
                  ...
                  selID = selID=selID[1];
                  if (Fragment.crossRefMap) {
                    Fragment.loadTarget(selID, {preBuffer:2,postBuffer:2,f:function() {rightSidePane.findTargetElement(e);}});
                    //var targetFrag = Fragment.getIntraRefFragId (selID);
                ...
                if (!$(wsObj)[0]) {
                    ...
                    if (Fragment.crossRefMap) {
                        ...
                        Fragment.loadTarget(selIdUnescaped, {preBuffer:2,postBuffer:2,f:function() {rightSidePane.findTargetElement(e);}});
    ...
    setWorkSpace:function(evt, selID) {
        ...
        var wsObjClass=$(wsObj).attr('class')?$(wsObj).attr('class'):$(wsObj).parent().attr('class');
        var classArr=wsObjClass.split(' ');
        if(classArr.length>1)wsObjClass=classArr[0];
        ...
        if(wsObjClass!='authorName') {
            ...
            data+= '<li><a href="#'+selID+'" class="js_article viewInArt"><span class="icon viewInArticle_'+SD_UTIL.getProdColor()+'"></span>View in article</a></li>';
            if(wsObjClass=='figure'){
                var downLink=$(wsObj).closest('dl').find('.menuButton li').eq(0).clone();
                if(downLink[0]){
                    downLink.prepend('<span class="icon viewFullsize"></span>');
                    downLink.find('a').text('View full-size image')
                    data+= downLink.outerHTML();
                }
            }
            if(wsObjClass=='ecomponent'){
              var obj='';
        ...
        if (wsObjClass=='authorName' && (SDM.contentType=='JL' || SDM.contentType=='BS')) {
            if ($(wsObj).attr("data-tb") != 'TRUE' && $(wsObj).attr("data-ln") != undefined && $(wsObj).attr("data-fn") != undefined && $(wsObj).attr("data-pos") != undefined && DynamicArtContent.dynamicComponents.scopoutAvail == "Y") {
                var requestObj = new Object();
        ...
        if(mathRenderer.opt.renderOn)mathRenderer.handleRerender($('#rightPane'));
        rightSidePane.showWorkspace(evt);
        rightSidePane.resetWorkspace();
        mathRenderer.activateButtons();
    },
    showWorkspace:function(obj){
        if($('#articleToolbar').hasClass('fixedPane')){
            var wsTop=$('#articleToolbar').outerHeight();
        }else{
            var wsTop=$('#articleToolbar').outerHeight()+$('#header-area').height();
        }
        $('.js_workspace_content').css('top',wsTop).show();
 
        $('.js_workspace_content .closeBt').focus();
        $('.js_workspace_content').off('click').on('click','.closeBt',function(e){
            e.preventDefault();
            if(typeof obj!='undefined' && typeof obj.currentTarget!='undefined')$(obj.currentTarget).focus();
            $('.js_workspace_content').hide();
        });
        $('.js_workspace_content').off('keydown','a:last')
            .on('keydown','a:last', function(e) {
            if (e.which == 9 && !e.shiftKey) {
                e.preventDefault();
                $(this).closest('.js_workspace_content').find(':tabbable').eq(0).focus();
            } 
        });
        $('.js_workspace_content').off('keydown','a:first')
                    .on('keydown','a:first', function(e) {
            if (e.shiftKey && e.which == 9) {
                e.preventDefault();
                $(this).closest('.js_workspace_content').find(':tabbable').eq(-1).focus();
            } 
        });
    },
    resetWorkspace:function(){
        $('.js_workspace_content:visible').each(function(){
            var rightPos=$(window).width()-$('#page-area').width()+$(window).scrollLeft();
            if($(window).width()>$('#page-area').width())rightPos=rightPos/2;
            $(this).css({'right': rightPos-1});
        });
    },
    buildPreviousNextLinks:function(baseElement,wsObj,wsObjClass){
      if(Fragment.isAvail()) {return rightSidePane.buildPreviousNextLinksFragment(baseElement,wsObj,wsObjClass);}
        ...
        if(next && $(next).find('dl.'+wsObjClass).length>0){
            next = $(next).find('dl.'+wsObjClass+':first');
        }
        if(prev && $(prev).find('dl.'+wsObjClass).length>0){
            if(indexVal-2>=0){
                prev = $(baseElement).eq(indexVal-2);
             }else{
                prev = undefined;
             }
        }
        ...
        if ($(next)[0] || $(prev)[0]) {
            ...
            if($(prev).attr("id")){
                data+='  <a href="#'+$(prev).attr("id")+'" class="previous S_C_prev">&laquo; previous '+ wsObjClass +'</a>';
            }
            if($(next).attr("id")){
                data+='  <a href="#'+$(next).attr("id")+'" class="next S_C_next">next '+ wsObjClass +' &raquo;</a>';
            }
            data+='</div>';
        }
        return data; 
    ...
    buildPreviousNextLinksFragment:function(baseElement,wsObj,wsObjClass) {
      ...
      if(wsObjClass=='figure' && (prev || next)){
        if(next && !$('#'+next)[0]){
            var targetFrag = Fragment.getIntraRefFragId (next);
            Fragment.get (targetFrag, undefined , {successCb: function(){
                if($('#'+next).find('dl.'+wsObjClass).length>0){
                    next = Fragment.getNextByType(next, wsObjClass);
                    $('#rightPane .buttonNavBox .next').attr('href','#'+next);
                }
            }});
        }else if($('#'+next).find('dl.'+wsObjClass).length>0){
            next = Fragment.getNextByType(next, wsObjClass);
        }
        if(prev && !$('#'+prev)[0]){
            var targetFrag = Fragment.getIntraRefFragId (prev);
            Fragment.get (targetFrag, undefined , {successCb: function(){
                if($('#'+prev).find('dl.'+wsObjClass).length>0){
                    prev = Fragment.getNextByType(prev, wsObjClass);
                    $('#rightPane .buttonNavBox .prev').attr('href','#'+prev);
                }
            }});
        }else if($('#'+prev).find('dl.'+wsObjClass).length>0){
            prev = Fragment.getPrevByType(prev, wsObjClass);
        }
      }
      ...
      if (next || prev) {
        data+='<div aria-label="Go to previous and next in sidebar" role="navigation" class="buttonNavBox">';
        if(prev){
          data+='  <a href="#'+prev+'" class="previous S_C_prev">&laquo; previous '+ wsObjClass +'</a>';
        }
        if(next){
          data+='  <a href="#'+next+'" class="next S_C_next">next '+ wsObjClass +' &raquo;</a>';
        }
        data+='</div>';
      }
      return data; 
            ...
            $.each ($(selObj).find("img.figure"), function(){
				...
				if(imgSrc == '/sd/grey_pxl.gif'){
				    if (dataGabsImgEID != undefined && dataGabsImgEID != '' && SDM.entitled) {
                        imgSrc = SDM.imageUrlPrefix + dataGabsImgEID;
                    }else if (dataFullImgEID != undefined && dataFullImgEID != '') {
                       imgSrc = SDM.imageUrlPrefix + dataFullImgEID;
                    }else if (dataThumbImgEID != undefined && dataThumbImgEID != '') {
                        imgSrc = SDM.imageUrlPrefix + dataThumbImgEID; 
                    }else if (dataInlImgEid != undefined && dataInlImgEid != '') {
                        imgSrc = SDM.imageUrlPrefix + dataInlImgEid; 
	                }
				}
				if(imgSrc!=undefined)data+='<dt><img src="'+imgSrc +'" alt="'+ $(this).attr("alt") +'" class="'+selObjClass+'"></dt>';
				});
            if($(selObj).find("dd p")[0])data+= ' <dd>'+$(selObj).find("dd p").outerHTML() +'</dd>';
            data += '</dl>';
        }
        return data;  
    },
    ...
    populateReferences:function(selObj,selObjClass){
        ...
        $(selObj).find("ul.reference").each(function(){
            if($(this).find("li.author").html()!=null) { data+= '<li class="author">'+ $(this).find("li.author").html() +'</li>'; }
            if($(this).find("li.title").html()!=null) { data+= '<li class="title">'+ $(this).find("li.title").html() +'</li>'; }
            if($(this).find("li.source").html()!=null) { data+=' <li class="source">'+$(this).find("li.source").html() +'</li>'; }
            data+='<ul class="placeHolder '+$(this).attr('id')+'"></ul>'; 
    ...
    populateReferenceLinks:function (selObj) {
        var url='';
        var placeHolderObjArr = $(selObj).find("li.refPlaceHolder");
		$(placeHolderObjArr).each(function(){
			var placeHolderObj=this;
			var ab=$(placeHolderObj).closest('ul').attr('id');
			if ($(placeHolderObj).attr("data-refres")=='Y') {
				// Reference has been resolved.
				rightSidePane.getAbstract(placeHolderObj,ab);
				if ($(placeHolderObj).html()!=null) {
					...
					var data='';
					data+='<ul class="links">'; 
					$(this).find('div.boxLink').each(function(){ 					
					...
					var obj = $(wsObj).find("ul.placeHolder."+ab);
					$(obj).show();
					$(obj).html(data);
					data='';
				}
			} else {
				// Reference not resolved. Resolve reference and get 
				rightSidePane.resolveRefsForWorkSpacePanel(placeHolderObj,ab);
			}
		});
    },
    getAbstract:function(refHolder,parentID){
        var absUrl = $(refHolder).attr("data-absUrl");
        if (absUrl == undefined) { return; }
        var ajaxReq = $.get(absUrl, function(response) {
            if (response.length > 1) {
                 var wsObj = $('.js_workspace_content').find('.scrollArea');
			     var obj = $(wsObj).find("ul.placeHolder."+parentID);
				 $(obj).prepend('<li>'+response+'</li>');
            }
        });
    },
            ...
            var ajaxReq = $.get(citeUrl, function(response) {
                if (response) {
                     ...
                     if ($(wsObj).find('.citedBy_')[0]) {
                         ...
                         $(citeObj).closest('div.boxLink').before('&nbsp;|&nbsp;');
                     }
                }
            });
       }
    },
    resolveRefsForWorkSpacePanel:function(placeHolderObj,parentID){
        ...
        if (obj && url) {
            var ajaxReq = $.get(url, function(response) {
                if (response) {
					...
					data+='<ul class="links">';
					$(response).find('div.boxLink').each(function(){
						var clonedObj = $(this).clone();	
						data+='<li class="refLinks">'+$(clonedObj).outerHTML()+'</li>';
					});
                                        data+='<ul>';
    ...
    populateAuthorInfo:function(selObj,selObjClass){
        ...
        var vitaeData='';
        var authText='';
        data+='<h3 class="workAuthor">' + $(selObj).closest('li').find('.authorName').text()+'&nbsp;'
        ...
        if(affData=='' && $(selObj).closest('ul').nextAll('ul.affiliation')[0]){
            var affData=$(selObj).closest('ul').nextAll('ul.affiliation').eq(0);
            var affOtherData='';
    ...
    showRightAd:function(obj) {
        ...
        if (rpWidth <= 300 && SDM.adArticleRightSmURL != "") {
            iframe = '<div id="articleRightAdSm" class="adBlock"><iframe scrolling="no" frameborder="0" border="0" cellspacing="0" src="' + SDM.adArticleRightSmURL + '"></iframe></div>';
        }
        else if (rpWidth > 300 && SDM.adArticleRightLgURL != "") {
            iframe = '<div id="articleRightAdLg" class="adBlock"><iframe scrolling="no" frameborder="0" border="0" cellspacing="0" src="' + SDM.adArticleRightLgURL + '"></iframe></div>';
        }
...
var GadgetUtil = {
  ...
  displayGadgetsWrapper:function() {
    if(typeof displayGadgets=='function') {
      ...
      displayGadgets();
      setTimeout(function(){
        $('#SD_BIB, #SD_CRA').each(function(){
            if($(this).children().length>0){
                $(this).prev('.gadSep').show();
            }
        });
        if(typeof ArticlePage != 'undefined')ArticlePage.viewHandler();
      }, 500);
...
$(document).ready(function(){
    ...
    $('body').on('click','.tbxCollapse',function(){textBoxCE('',this);});
    $('body').on('click','a.alphaAnchor,a.glossaryAnchor',function(e) {
        var url=$(this).attr('data-url');
        var txt=$(this).text();
        loadSection(url,txt);
    });
    $('body').on('click','#ddDoi',function(e) { 
        var doiWin;
        var txt=$(this).attr('data-url');
        doiWin=window.open('http://dx.doi.org/'+txt,'doilink','scrollbars=yes,resizable=yes,directories=yes,toolbar=yes,menubar=yes,status=yes');
        doiWin.focus();
    });
    $('body').on('click','.iconAnchor',function(e) { 
        var helpWin;
        var txt=$(this).attr('data-url');
        helpWin=window.open(txt+'/science/help/doi.htm','sdhelp','scrollbars=yes,resizable=yes,directories=no,toolbar=no,menubar=no,status=no,width=760,height=570');
        helpWin.focus();
    });

    $('body').on('click','.S_C_full_size,.S_C_high_quality',function(e) { 
        var src=$(this).attr('data-src');
        var labelCaption=$(this).attr('data-label');
        var captionRef=$(this).attr('data-ref');
        var format=$(this).attr('data-format');
        openStrippedNS(src,labelCaption,captionRef,format);
        return false;
    });
    $('body').on('click','.videoclip',function(e) { 
        openNS(this.href,700,500);
        return false;
    });
    
    $('body').on('click','.tableAnchor',function(e) { 
        var url=$(this).attr('data-url');
        openStrippedNS(url,'');
        return false;
    });
...
var mathRenderer={
    ...
    init:function(){
        this.opt.config = 'MathJax.Hub.Config({' +
            ...
            '    $(window).lazyLoadMathJax({mmlSel:".mathContainer"});  '+
		...
		if($(obj).attr('id')=='leftPane' || $(obj).attr('id')=='rightPane'){
                ...
                $(obj).find('span.MathJax').each(function(){
...
var mrwLeftPane={
    ...
    init:function(){
        $('#outlineContainer').prepend('<ul id="lpTabs">'
            ...
            + '<li class="rightTab tab S_C_articleOutline" tabindex="0">Article Outline</li>'
            + '</ul>');
        $('#lpTabs .leftTab').click(function() {
        ...
        $('#outline, #outlineContainer, #leftPaneInner').addClass('book');
        ...
        //$('#outlineContainer').prepend('<div id="lpTabs" style="height:10px;">&nbsp;</div>');
    },
    load:function(){
        $('body').append('<ul id="srcOutline"></ul>');
    ...
    showSubject:function(e) {
        ...
        $('#olGraphCbBox').hide();
        $('#outline').css('top', 27);
        $('#itemOutline,#fig').hide();
        mrwLeftPane.currentTab='S';
    },
    showOutline:function(e) {
        $(e).parent().children('.tab').removeClass('activeTab');
        $(e).addClass('activeTab');
        $('#itemOutline,#fig').show();
        $('.outlineMsg').show();
        $('#srcOutline').hide();
        $('#olGraphCbBox').show();
        mrwLeftPane.currentTab='O';
        if($('#itemOutline').text()!='')$('#outline.book').css('top', 27);
        if($('#outlineGraphicsCheckBox').prop('checked')==true) {
            Outline.toggleGraphics('show');
  ...
  get: function(id, opts, i) {
    ...
    if(i==0) {
      ...
      cfg.msgTimeout = setTimeout(function() {
        ...
        $('#leftPaneInner.fixedPane, .topBorder.fixedPane, .appContainer.fixedPane').css('top', $('#articleToolbar').outerHeight());
        $('.js_workspace_content').css('top', $('#header-area').height()+$('#articleToolbar').outerHeight());
      }, cfg.msgDelay);
    }
    $.get(cfg.url)
      .success(function(a,b,c) {
...
var blockCollab=false;
var MoreInformation = {
  onDemand: true,
  closed: true,
  hideFrag: function($f) {
    if($f.attr('id') == 'frag_1') return true;
    if($f.find('.moreLink').length) return true;
    if ( $f.find('ul.authorGroup').length || 
         $f.find('ul.affiliation').length ){
      $f.attr('data-mi', '1');
      return false;
    } else {
      return true;
  ...
  fragLoad: function(obj) {
    var current = obj.c;
    var start   = obj.s;
    var len     = obj.l;
    var func    = obj.f;
    $.ajax({type:'GET',
            url:SDM.urlPrefix + "/frag/" + SDM.pm.pii + "/" + SDM.pm.fat + "/" + "batch" + "/" + obj.s + "/" + obj.l,
            success: function(str,b,c) {
              str.length > 30 ? sI=-30 : sI=str.length*-1;
              var endStr = str.slice(sI);
              if(endStr.match(/ERROR::/)) {
                DBG.out(0, "MoreInformation::fragLoad:: Unable to retrieve batch");
                return;
              }
              var parts = str.split('!!!FRAG!!!');
              var partCount = parts.length-1;
              DBG.out(0, "MoreInformation::fragLoad:: received " + partCount + " fragments");
              for(var i=0; i<partCount; i++) {
                DBG.out(1, "MoreInformation::fragLoad::working on " + (start+i));
                var fragIdNum = start+i;
                if(!$('#frag_'+fragIdNum).length) {
                  var prevFrag = Fragment.getPrevFrag(fragIdNum);
                  DBG.out(1, "Inserting after " + prevFrag.attr('id'));
                  ...
                  $('#frag_' + fragIdNum).hide();
                  Fragment.processFrag($('#frag_' + fragIdNum), true);
                  $('#frag_' + fragIdNum).children('ul.authorGroup').addClass('smh').css('display', 'inline');
                  $('#frag_' + fragIdNum).find('ul.affiliation').addClass('smh');
                }
                else {
                  DBG.out(0, "ignoring frag_" + fragIdNum);
                }
  ...
  onDemandLoad: function(func) {
    if(MoreInformation.onDemand) {
      var begin = MoreInformation.calculateFid();
      var end = Fragment.foldCheckJumper();
      var len = end - begin;
      DBG.out(0, 'MoreInformation::onDemandLoad()::begin::' + begin + '\tend::' + end + '\tlen::' + len);
      if(begin==-1 || len < 1) {
        DBG.out(1, 'Nothing to load');
        func();
      }
      else {
        DBG.out(1, 'Need to load ' + begin + ',' + len);
        MoreInformation.fragLoad({c:begin, s:begin, l:len, f:func});
      }
    }
  },
  cleanFrag: function(fid) {
    DBG.out(1, 'MoreInformation.cleanFrag(' + fid + ')::');
  },
  calculateFid: function() {
    var end = Fragment.foldCheckJumper();
    var i = 2;
    while( $('#frag_' + i).length && i<end ) {
      i++;
    }
    if(i>=end)
      i=-1;
  ...
  init: function() {
    ...
    if(MoreInformation.onDemand) {
      ...
      if( Fragment.isAvail() ) {
        fid = MoreInformation.calculateFid();
      }
      else {
        fid = -1;
      }
    ...
    if(fid!=-1) {
      DBG.out(0, 'MoreInformation fetching frag_' + fid);
      Fragment.get('frag_' + (fid + 0), undefined, {successCb: function(fi) {MoreInformation.cleanFrag(fi);MoreInformation.init();},
                                                    successCbArg: fid,
                                                    showLoadedFrag: MoreInformation.hideFrag
                                                   });
    }
      ...
      //CrossMark
      if (SDM.isCrossMark) {
        htmlStr +='<dd class="crossMarkInfo smh">';
        htmlStr += '<div>';
      ...
      //OA sponsor
      if (SDM.oaSponsor != '') {
        htmlStr += '<dd class="oaSponsor smh">' + SDM.oaSponsorLabel + SDM.oaSponsor + '</dd>';
      };
      //OA user license
      if (SDM.oaUserLicense != '') {
        var OAuserLicense = decodeURIComponent(SDM.oaUserLicense);
        if (OAuserLicense.match(/^http/i) != null) {
          //If this is a URL, we will build a link.
          htmlStr += '<dd class="oaUserLicense wrap2 smh">License: &nbsp;';
          htmlStr +=    '<span>';
          htmlStr +=     '<a id="ddOAuserLicense" href="' + OAuserLicense + '" target="_blank">' + decodeURIComponent(OAuserLicense) + '</a>';
          htmlStr +=     '</span>';
          htmlStr += '</dd>';
        }
        else { 
          htmlStr += '<dd class="oaUserLicense wrap2 smh">';
          htmlStr +=    '<span>';
          htmlStr +=  OAuserLicense;
...
//      htmlStr += '<dd class="permissions smh"></dd>';
      htmlStr +='</dl>';

      $('#centerInner .moreInformation').html(htmlStr);

      if($('#ppvPlaceHolder').length && SDM.pageType=="article_full"){$('#ppvPlaceHolder').hide();}

      $('.articleDates').addClass('smh');
      $('.preprint').addClass('smh');
      //crossmark
      $('.moreInformation').addClass('smh');
      $('.miscInfo').addClass('smh');
//      $('.permissions').addClass('smh');

      if(Fragment.isAvail()) {
        $('.page_fragment').children('ul.authorGroup').addClass('smh').css('display', 'inline');
        $($('.page_fragment').children('ul.authorGroup')[0]).removeClass('smh');
        $('.page_fragment').find('ul.affiliation').addClass('smh');
      }
      else {
        $('#centerInner').children('ul.authorGroup').addClass('smh').css('display', 'inline');
        $($('#centerInner').children('ul.authorGroup')[0]).removeClass('smh');
        $('#centerInner').find('ul.affiliation').addClass('smh');
      }

      var btnHtmlStr = '<div class="showLess moreLink">'
                     +   '<span class="showInfo expand" title="Show more author and article information"> Show more </span>'
                     + '</div>'
                     + '<div class="showLess lessLink ">'
                     +   '<span class="showInfo collapse" title="Hide more author and article information"> Show less </span>'
                     + '</div>';

      $('#showMoreButtons').html(btnHtmlStr);
      OpenAccess.init();

      if($('#currentAsOf').html.length>0){
        $('<div id="currtAsOf"></div>').insertAfter($('.lessLink'));
        $('#currtAsOf').html($('#currentAsOf').html());
      }

      if(!SDM.ignoreForceAbst){
        $('#ppvPlaceHolder').show();
      }
      if(SDM.entitled && !SDM.forceAbst) {
        $('#ppvPlaceHolder').addClass('smh');
      }
      
      $('.moreLink').before($('#ppvPlaceHolder'));
  
      $('.lessLink').click(function() {
        MoreInformation.showLessInfo();
        return false;
      });
      $('.moreLink').click(function() {
        if(MoreInformation.onDemand && Fragment.isAvail()) {
          MoreInformation.onDemandLoad(function() {MoreInformation.showMoreInfo();ArticlePage.viewHandler();});
        }
        else {
          MoreInformation.showMoreInfo();
          ArticlePage.viewHandler();
        }
        return false;
      });
      ...
      $('.smh').hide();
      $('dl.doiLink').parents('.page_fragment').show();


      if(typeof ArticlePage != 'undefined')ArticlePage.setPaneWidths();
      Fragment.scrollFlag = true;
    }
  },
  showLessInfo : function(){
    Fragment.scrollFlag = false;
    MoreInformation.closed = true;
    $('.moreLink').show();
    $('.lessLink').hide();
    $('.smh').hide();

    if(Fragment.isAvail()) {
      var fList = $('#centerInner .page_fragment').filter( function() { return !MoreInformation.hideFrag($(this)); } );
      fList.hide();
      //if($('li.collabWithAuthorCollapse a.collabText').hasClass('collabClosed')) {
      //  $('.hideShow').css('display','block')
      //}
      ArticlePage.viewHandler();
  ...
  showMoreInfo : function(){
    ...
    if(Fragment.isAvail()) {
      var fList = $('#centerInner .page_fragment').filter( function() { return !MoreInformation.hideFrag($(this)); } );
      fList.show();
      if($('li.collabWithAuthorsCollapse a.collabText').hasClass('collabOpened')) {
        $('.hideShow').css('display','block')
      }
    }
    return;
...
$.fn.ieWarning = function(){
    ...
    function showMsg(){
        container.show();
        $('.closeBt', container).on('click', disableMsg);
  ...
  init: function() {
    if (SDM.isOAarticle && !SDM.isOAtitle && !($('span.articleOAstatus')[0]) ) {
      if ($('hr.artHeader')[0]) {
        ...
        if (SDM.isOAopenArchiveArticle) {
           $('.OAlabel').prepend('<span class="articleOAstatus">&nbsp;&nbsp;Open Archive</span>');
        } else {
           $('.OAlabel').prepend('<span class="articleOAstatus">&nbsp;&nbsp;Open Access</span>');
        }
      } else {
        if (SDM.isOAopenArchiveArticle) {
           $('.articleOAlabelForced').html('<span class="articleOAstatus">&nbsp;&nbsp;Open Archive</span><hr/>').show();
        } else {
...
function setQuickSearch(){
    var winWidth=$(window).width()-16;
    if(winWidth<=1024){
        $('.quicksearch').addClass('low');
    }else{
        $('.quicksearch').removeClass('low');
    }
}
var prodMsg={
    init:function(){
        var sdMsg = getCookie('sd_system_msg');
        if(!sdMsg){
            $('#sdSystemMsg').show();
            $('#centerInner').on('click','#sdSystemMsg .closeBt',function(e){
                prodMsg.close(e)
            });
        }else{
            $('#sdSystemMsg').remove();
    ...
    close:function(e){
        var msgBlock=$('#sdSystemMsg');
        msgBlock.animate({height:0},200,"linear", function(){
            msgBlock.remove();
            createCookie('sd_system_msg', 1);
        });
    }
};
var adDisplay={
    init:function(){
        $(document).off('click').on('click','.adText .closeBt',function(e){
            adDisplay.close(e)
        });
    },
    close:function(e){
        var adBlock=$(e.target).closest('.adBlock');
        if(adBlock.attr('id')=='articleLeftAd'){
            adBlock.animate({height:0},200,"linear", function(){
                adBlock.remove();
                createCookie('ad_left', 1);
            });
        }else if(adBlock.attr('id')=='articleRightAdLg' || adBlock.attr('id')=='articleRightAdSm'){
            adBlock.animate({height:0},200,"linear", function(){
                adBlock.remove();
                createCookie('ad_right', 1);
            });
        }
    }
};
...
var exportCite = {
    ...
    init: function() { 
        cookieName = getCookie('exportCookie');
        
        if(cookieName){
            finalExportCookie = cookieName.split("</q>");
            splitCookie = finalExportCookie[1];
            firstExportCookie = splitCookie.split("<enc>");
            EXPORT_NAME = firstExportCookie[0];
            if(EXPORT_NAME=="RIS") {
              $('.export_general').val("Export to RIS");
              $('.export_general').attr('title','Export to RIS');
              $('.export_icon').attr('title','Export to RIS');
            } else if(EXPORT_NAME=="ASCII") {
                $('.export_general').val("Export to plain text");
                $('.export_general').attr('title','Export to plain text');
                $('.export_icon').attr('title','Export to plain text');
            } else if(EXPORT_NAME=="BIBTEX") {
                $('.export_general').val("Export to BibTex");
                $('.export_general').attr('title','Export to BibTex');
                $('.export_icon').attr('title','Export to BibTex');
            } else if(EXPORT_NAME=="REFWX"){
                $('.export_general').val("Save to Refworks");
                $('.export_general').attr('title','Save to Refworks');
                $('.export_icon').attr('title','Save to Refworks');
                $('.export_icon').removeClass('initialImage');
                $('.export_icon').addClass('bgRefworks_Change');
                $('.RIS').attr('checked','checked');
            } else{
                $('.export_general').val("Save to Mendeley");
                $('.export_general').attr('title','Save to Mendeley');
                $('.export_icon').attr('title','Save to Mendeley');
                $('.export_icon').removeClass('initialImage');
                $('.export_icon').addClass('bgMendeley_Change');
            }
        }
         $('.export_general,.exportDivision .export_icon').on('click keypress', function(e){
            if((e.keyCode && e.keyCode!=13))return;		 
            if($(this).attr('title')=="Export Citation"){
                $('input.zone').attr('value','exportDropDown');
                exportPopUp(e);
            }
            else{
                $('input.zone').attr('value','toolbar');
                var exportFormat=$(this).attr('title');
                if(exportFormat!="Save to Mendeley")
                {
                    if (exportFormat=="Export to plain text")
                        $('.ASCII').attr('checked','checked'); 
                    else if(exportFormat=="Export to BibTex")
                        $('.BIBTEX').attr('checked','checked'); 
                    else if(exportFormat=="Save to Refworks"){
                        $('.REFWX').attr('checked','checked');exportRefworks=true;}
                    else 
                        $('.RIS').attr('checked','checked');
                    $(e.currentTarget).closest('.pulldown').find('.menu .export_button').trigger('click');
                }
                else {
                mendeleyZone='toolbar';
                directExport(e);
                }
            }
        }); 
        ...
        $(document).keyup(function(e){ 
            ...
            if($('#export_popup:visible')) 
                {
                    $('.export_popup').addClass('hidden');
                    $('.dropdownArrow').attr('aria-expanded', 'false');
                    $('.exportDivision .dropdownArrow').removeClass('openDropdown');
                    $('.exportDivision span.downSM_'+ SD_UTIL.getProdColor()).removeClass("upSM_" + SD_UTIL.getProdColor());
                }
            } 
        });
        $('body').on('click',function(e) { exportPopupClose(e);});
        $('.export_popup .warningIcon').hide(); 
        $('body').on('click','.export_button', function(e){
        var zone = $("input[type='hidden'][name='zone']").val();
        internalExport(e);
        if(zone!="toolbar"){
            $('input.zone').attr('value','exportDropDown');
        }});
        $('body').on('click','.directExport', function(e){
        mendeleyZone='exportDropDown';
        directExport(e);});
        $('body').off('click.refworks').on('click.refworks','.refworks', function(e){
        refworksClick(e);});
        $('body').on('click','.exportDivision .dropdownArrow',function(e){$('input.zone').attr('value','exportDropDown'); exportPopUp(e)});
        exportCite.config.exportCountValue=$("input[type='hidden'][name='art']").length + $("input.checkBox:checked:visible").length ;
    },
    Mendeley: function(){
        MendeleyImporterApi.registerHostId("com.sciencedirect");
        ...
        MendeleyImporterApi.registerDataCallback(function(bookmarklet) {
                  return getExportInfo(bookmarklet);});
    }
};

function exportPopUp(event) {
    var menu=$('.export_popup');
    var aria=$('.dropdownArrow');
      if (aria.attr('aria-expanded')=='true'){
        ...
        $('.exportDivision .dropdownArrow').addClass('openDropdown');
        $('.exportDivision span.downSM_'+ SD_UTIL.getProdColor()).addClass("upSM_"+ SD_UTIL.getProdColor());
      }
      if($('.export_general').val()=="Save to Refworks")$('.RIS').attr('checked','checked');
}
function exportPopupClose(e)
{
 if( typeof e == "undefined" || $(e.target).closest('.pulldown').length===0)
        {
            if($('#export_popup:visible'))
                {
                    $('.export_popup').addClass('hidden');
...
function formSubmit() {
$('.expContainer').closest('form').off('submit').on('submit',function(){ 
if(exportRefworks)
{
$(this).attr('target','_blank');
exportRefworks=false;
} 
else
{
    if(exportClicked){
...
function internalExport(e)
{ 
  exportCite.config.EXPORT_NAME=$(e.currentTarget).closest('.menu').find('.export_radio input:radio:checked').val();
  $('.export_popup').addClass('hidden');
  $('.dropdownArrow').attr('aria-expanded', 'false');
  $('.exportDivision .dropdownArrow').removeClass('openDropdown');
  $('.exportDivision span.downSM_'+ SD_UTIL.getProdColor()).removeClass("upSM_" + SD_UTIL.getProdColor());
  createCookie("exportCookie",exportCite.config.EXPORT_NAME);
  if(exportCite.config.EXPORT_NAME=="RIS"){
        $('.export_general').val("Export to RIS");
        $('.export_general').attr('title','Export to RIS');
        $('.export_icon').attr('title','Export to RIS');
        $('.export_icon').addClass('initialImage').removeClass('bgMendeley_Change bgRefworks_Change');
  }else if(exportCite.config.EXPORT_NAME=="ASCII"){  exportClicked=true;
        $('.export_general').val("Export to plain text");
        $('.export_general').attr('title','Export to plain text');
        $('.export_icon').attr('title','Export to plain text');
        $('.export_icon').addClass('initialImage').removeClass('bgMendeley_Change bgRefworks_Change');
  }else if(exportCite.config.EXPORT_NAME=="REFWX"){
        $('.export_general').val("Save to Refworks");
        $('.export_general').attr('title','Save to Refworks');
        $('.export_icon').attr('title','Save to Refworks');
        $('.export_icon').addClass('bgRefworks_Change').removeClass('initialImage bgMendeley_Change');
  }
  else if (exportCite.config.EXPORT_NAME=="BIBTEX"){
        $('.export_general').val("Export to BibTex");
        $('.export_general').attr('title','Export to BibTex');
        $('.export_icon').attr('title','Export to BibTex');
        $('.export_icon').addClass('initialImage').removeClass('bgMendeley_Change bgRefworks_Change');
...
function directExport(event) 
{
    $('.export_popup').addClass('hidden');
    $('.dropdownArrow').attr('aria-expanded', 'false');
    $('.exportDivision .dropdownArrow').removeClass('openDropdown');
    $('.exportDivision span.downSM_'+ SD_UTIL.getProdColor()).removeClass("upSM_" + SD_UTIL.getProdColor());
    createCookie("exportCookie","Mendeley");
    $('.export_general').val("Save to Mendeley");
    $('.export_general').attr('title','Save to Mendeley');
    $('.export_icon').attr('title','Save to Mendeley');
    $('.export_icon').removeClass('initialImage bgRefworks_Change');
    $('.export_icon').addClass('bgMendeley_Change');
    MendeleyImporterApi.open();
    exportCite.Mendeley();
}

function refworksClick(event)
{
    $(event.currentTarget).closest('form').attr('target','_blank');
    exportCite.config.EXPORT_NAME='REFWX';
    exportRefworks=true;
    $('.REFWX').attr('checked','checked');
    $('.export_popup').addClass('hidden');
    $('.dropdownArrow').attr('aria-expanded', 'false');
    $('.exportDivision .dropdownArrow').removeClass('openDropdown');
    $('.exportDivision span.downSM_'+ SD_UTIL.getProdColor()).removeClass("upSM_" + SD_UTIL.getProdColor());
    createCookie("exportCookie","REFWX");
    $('.export_general').val("Save to RefWorks");
    $('.export_general').attr('title','Save to RefWorks');
    $('.export_icon').attr('title','Save to RefWorks');
    $('.export_icon').removeClass('initialImage bgMendeley_Change');
    $('.export_icon').addClass('bgRefworks_Change');
    $(event.currentTarget).closest('.pulldown').find('.menu .export_button').trigger('click');
}

//Mendeley Call back API for Encyrypted authtoken retrieval

function getUserIdentity()
{
  var arraydata = [];
  var userIdentityUrl = "/science/export/userIdentity";
  
  if(mendeleyZone){
    userIdentityUrl =userIdentityUrl+"?zone="+mendeleyZone;
  }
  ...
  jQuery.ajax(
  {
    ...
    {
      arraydata["hash"]=data.hash;
      delete data.hash;
      arraydata["json"]=JSON.stringify(data);
    },
    async:false,
    error:function(d) 
    {
        DBG.out(0, "Error on getting Encrypted AuthToken");
    }
  });    
  return arraydata;
}

//Mendeley Call back API for EID retrieval
function getExportInfo(bookmarklet)
{ 
  var jsonToMendeley=[]
  var piiJson="{\"pii\":\"" +SDM.pm.pii +"\"}";
  jsonToMendeley[0] = $.parseJSON(piiJson);    
  return jsonToMendeley;
}

// Getting the citedByScopus count
function getcitedByScopusCount(){
 var citedByURL = SDM.urlPrefix +'/citedby/rslts/'+SDM.pm.pii+'/hitcount';
 try {
     $.get(citedByURL, function(res) {
        res = $.trim(res);
        if(res && res.length>0) {
            if ($('#rightPane span#hitcount')[0]) {
                $('#rightPane span#hitcount').text(res);
            }
         }
     });
 }
