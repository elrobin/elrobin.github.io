...
function onEditClick(key,url,features,e)
{e=getEvent(e);var sender=getSender(e);if(sender==null)
return false;if(sender.tagName=='A')
return true;if(editableControl_checkLock(key,getRootId(sender)))
{alert('This item is currently being edited by another user.');return false;}
if(window.showModalDialog)
{window.showModalDialog(url,'_blank',features);window.location=window.location;}
else
window.open(url,'_blank',features+', modal=true');}
function getRootId(element)
{while(element!=null)
{if(getAttributeValue(element,'key')=='editableControl')
return element.id;element=getParent(element);}}
function editableControl_startLockChecking(key,id)
{setTimeout('editableControl_onLockCheck("'+key+'", "'+id+'")',1*60*1000);}
function editableControl_onLockCheck(key,id)
{editableControl_checkLock(key,id);setTimeout('editableControl_onLockCheck("'+key+'", "'+id+'")',1*60*1000);}
function editableControl_checkLock(key,id)
{var isLocked=MetaPress.Products.Reader.Web.SiteEditor.EditableControl.IsLocked(key).value;var element=getElement(id);if(element==null)
alert('Key: '+key+'\nId: '+id);if(isLocked)
{element.className='lockedEditableRegion';}
else
{element.className='editableRegion';}
return isLocked;}
...
{addHandler(fieldList.childNodes[k],'onclick',queryBuilder_onFieldClick);addHandler(fieldList.childNodes[k],'onmouseover',queryBuilder_onMouseOver);addHandler(fieldList.childNodes[k],'onmouseout',queryBuilder_onMouseOut);}}
...
else if(getAttributeValue(operatorList.childNodes[k],'arg')!='')
