...
var DynamicArtContent = {
  ...
  buildAccessLinks: function(scope) {
    ...
    if(outLinkHTML[5]!=''){
        ...
        $('.moreInformation .permissions').html(outLinkHTML[5]);
    }else{
...
var MoreInformation = {
  ...
  init: function() {
    ...
    if(fid!=-1) {
      ...
      $('#centerInner .doiLink').html(doiHtml);

      var $doiLink = $('.doiLink');
      var $ppvHolder = $('#ppvPlaceHolder');

      if($ppvHolder && SDM.pageType=="article_full"){$ppvHolder.hide();}

      //$($('.authorGroup')[0]).addClass('firstAuthFrag');
